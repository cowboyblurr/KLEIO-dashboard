...
export default function SubmissionsPage() {
  ...
  const filteredSubmissions = useMemo(() => {
    ...
    if (!normalized || normalized === "material" || normalized === "review") return allSubmissions
      ...
      <main className="h-full overflow-auto px-6 py-6">
        <div className="mx-auto min-w-[760px] max-w-[1180px] space-y-5">
          ...
          <section className="rounded-2xl border bg-white p-5" style={cardStyle}>
            <SearchFilterBar placeholder={t("institution.workspace.submissions.searchPlaceholder")} value={query} onChange={setQuery} suggestions={suggestions} filterChips={["All Programs", "All Statuses", "Material Readiness", "Review Stage"]} />
            <p className="mt-3 text-sm" style={{ color: mutedColor }}>
              Showing {filteredSubmissions.length} of {analytics.totalApplications} submissions across {analytics.activePrograms} programs.
            </p>
          </section>
