...
const simulations = [
  ...
  ["S08", "Artwork can be added without leaving application", () => { must(s.media, /ArtistImportStudio/, "Artwork quick-add must remain in application."); must(s.media, /without leaving this application|return you to this application/i, "Artwork recovery must explain route continuity.") }],
  ["S09", "Generic duplicate requirement upload is removed", () => { never(s.media, /QuickMediaImport|Add requirement file/i, "Artwork bar must not duplicate requirement upload."); must(s.requirementMedia, /ApplicationRequirementFilePicker/, "Exact requirement row must own its file picker.") }],
...
console.log("KLEIO artist beta convenience gate passed: application work stays on the canonical route; redundant uploads and page jumps are constrained; explicit source input type determines the correct control; mobile core actions stay visible; recovery remains in context; and external exits are reserved for the artist's actual submission destination.")
