...
requirePattern(mediaPanel, /Gemini places findings in the matching Passport fields/, "Document guidance must explain field-level placement.")
requirePattern(mediaPanel, /loadPassportReviewCount/, "The compact document row must show a relevant pending-review count.")
...
forbidPattern(mediaPanel, /rounded-\[24px\]|p-5 shadow-\[0_18px_52px/, "The document utility must not become an oversized hero again.")
requirePattern(passportPage, /pt-2/, "The Passport page must keep document tools close to the work surface.")
...
requirePattern(editor, /data-passport-field/, "Gemini findings must render inside the matching Passport field.")
...
forbidPattern(editor, /setRecord\([^\n]*proposed_value|update\([^\n]*proposed_value/, "Model proposals must never be inserted into the saved Passport state before approval.")
forbidPattern(editor, /rounded-\[24px\]|shadow-\[0_18px_48px/, "The Passport editor must not regress to oversized boxed sections.")

requirePattern(inbox, /Review information by field/, "The fallback review page must remain field-oriented.")
...
const artistRoots = [
  path.join(root, "app", "artist-dashboard"),
  path.join(root, "components", "kleio"),
]
...
console.log(`KLEIO artist relevance audit passed. Field-level Gemini review, compact Passport hierarchy, collapsed secondary controls, and shared workspace density are protected. ${oversizedSignals.length} high-density source files remain below the review threshold.`)
