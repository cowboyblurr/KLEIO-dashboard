...
requireText(documents, "Upload and analyze", "artist must explicitly initiate private analysis")
...
requireText(documents, "Pages analyzed", "completed analysis must expose page coverage")
requireText(documents, "Sections found", "completed analysis must expose perceived structure")
requireText(documents, "Conflicts", "completed analysis must expose conflicts")
requireText(documents, "Unresolved", "completed analysis must expose uncertainty")
...
console.log("Artist beta import audit passed: direct private PDF upload is the active source, Gemini receives original PDFs through a stateless server adapter, analysis results and limitations are visible, drafting uses artist-approved evidence, deferred providers remain gated, and no provider secrets are exposed.")
