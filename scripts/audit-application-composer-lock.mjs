...
const deliveryClient = read("lib/kleio-application-delivery.ts")
const deliveryMigration = read("supabase/migrations/20260808160000_lock_submission_delivery_to_finalized_version.sql")
...
forbidText(composer, /const href = `mailto:/, "The Application Composer must not construct an untracked raw mailto handoff directly.")
requireText(composer, /application dossier/, "The finalized application must have a portable professional dossier output.")

requireText(deliveryClient, /createRecipientReviewAccess/, "Tracked email delivery must use the hardened recipient-access boundary.")
requireText(deliveryClient, /buildMailtoHref/, "The manual beta fallback must automatically include the secure Review Room in the email handoff.")
requireText(deliveryClient, /record_my_application_delivery/, "All delivery adapters must feed the canonical delivery record.")
requireText(deliveryMigration, /application_recipient_access[\s\S]*submission_version_id/, "Recipient access must be explicitly bound to an immutable submission version.")
requireText(deliveryMigration, /bind_recipient_access_to_finalized_submission/, "The database must enforce immutable submission binding for every recipient handoff.")
requireText(deliveryMigration, /kleio_recipient_snapshot_from_submission_version/, "Review Room content must be rebuilt from the sealed submission snapshot.")
requireText(deliveryMigration, /create table if not exists public\.application_deliveries/, "Delivery must have a durable channel-agnostic state record.")
requireText(deliveryMigration, /provider_accepted/, "Delivery evidence must distinguish provider-confirmed acceptance from artist-reported sending.")
...
console.log("KLEIO application composer audit passed: Creative Passport reuse, opportunity-specific evidence grounding, portfolio relevance, exact-question drafting, artist control, preflight validation, immutable submission seals, immutable-version-bound tracked delivery, truthful external evidence, portable dossier output, recipient review continuity, unified application history, lightweight outcome updates, and application-answer cost metering are structurally present.")
