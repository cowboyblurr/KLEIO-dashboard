...
requireText(passport, "<MediaIntelligenceSheet", "Creative Passport must surface Media Assist in place")
requireText(passport, "loadMediaIntelligenceStatuses", "Creative Passport must reuse prior private Media Assist source results")
requireText(passport, ">Media Assist<", "Creative Passport must visibly use the Media Assist name")
...
requireText(library, "<MediaIntelligenceSheet", "Media Library must run Media Assist on supported sources in place")
requireText(library, "Run Media Assist", "Media Library must provide an explicit Media Assist action")
forbidText(library, 'href="/artist-dashboard/import/"', "Media Library must not force a separate PDF processing detour")
requireText(intelligence, "Run Media Assist", "shared Media Assist must provide an explicit artist action")
requireText(intelligence, "does not score the work, decide its meaning", "Media Assist must explicitly preserve artist authority over creative meaning")
requireText(intelligence, "edit, keep, or ignore", "Media Assist output must remain an optional artist-controlled suggestion")
forbidText(intelligence, "Overall review confidence", "Media Assist must not expose creative-facing confidence scoring")
forbidText(intelligence, "review confidence", "Media Assist must not expose creative-facing confidence scoring")
requireText(intelligenceClient, "requestMediaExtraction", "PDF source processing must retain the structured document engine")
requireText(intelligenceClient, 'functions.invoke("analyze-artist-media"', "image/video/audio processing must use the protected media analyzer")
...
console.log("Universal Media Import audit passed: private image/document/video/audio upload is active, live artist surfaces stay device/private-library first, missing application artwork can become a canonical Portfolio work without a route detour, Media Library and Creative Passport share in-place Media Assist with artist-controlled suggestions and no creative confidence scoring, unreleased connector internals remain gated, and owner-scoped RLS and secret hygiene are intact.")