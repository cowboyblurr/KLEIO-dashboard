...
const attributionTable = migration.match(/create table if not exists private\.artist_acquisition_attribution \(([\s\S]*?)\n\);/)?.[1] || ""
...
requirePattern(attributionTable, /artist_user_id uuid primary key/, "Attribution must be keyed to the authenticated artist privately.")
requirePattern(attributionTable, /acquisition_source text not null/, "Attribution must store one normalized source category.")
requirePattern(attributionTable, /anonymous_session_id uuid/, "Attribution may retain the random browser-session UUID for auditability.")
...
forbidPattern(attributionTable, /referrer|utm_|campaign|url|query_string|email|display_name/i, "Attribution table must contain only an approved category and random session UUID.")
