...
as $$
  ...
  where opportunity_row.status in ('open', 'forecasted', 'upcoming')
    ...
    and (
      ...
      or (
        cardinality(opportunity_row.eligible_countries) > 0
        and lower(trim(eligible_country)) = any(
          select lower(country_value)
          from unnest(opportunity_row.eligible_countries) country_value
        )
      )
    )
...
comment on function public.search_opportunities(text, text[], text[], text[], text, text[], boolean, boolean, integer, integer)
is 'Searches active, non-duplicate, reviewable opportunities. Unknown fees and unknown country eligibility never satisfy explicit no-fee or country filters.';

update public.opportunities
set deadline_at = null,
    recurring = true,
    updated_at = now()
where deadline_at >= timestamptz '2090-01-01 00:00:00+00'
  and status in ('open', 'forecasted', 'upcoming');
