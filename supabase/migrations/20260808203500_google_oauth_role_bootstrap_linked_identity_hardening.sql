-- A fresh institution OAuth bootstrap may only mutate the role when the
-- Supabase account itself was born as a Google-only identity. If Google was
-- auto-linked onto an email/password account, that existing KLEIO account role
-- is authoritative even during the short fresh-account window.
...
declare
  ...
  non_google_identity_exists boolean;
...
begin
  ...
  select
    ...
    (select count(*)::integer from auth.identities i where i.user_id = caller_id),
    exists (
      select 1 from auth.identities i
      where i.user_id = caller_id and i.provider <> 'google'
    )
  into current_role, onboarding_done, user_created_at, google_identity_exists, identity_count, non_google_identity_exists
  ...
  -- Correct-role returning accounts never need a mutation. This includes
  -- verified-email accounts that Supabase safely auto-linked to Google.
  if current_role = desired_role then
    return query
      select current_role, false, true;
    return;
  end if;

  -- A role mutation is reserved for a Google-created account, never an
  -- existing email/password account that acquired Google as another identity.
  if identity_count <> 1 or non_google_identity_exists then
    raise exception 'linked_account_role_mismatch';
  end if;
...
comment on function public.claim_fresh_google_signup_role(text) is
  'One-time role bootstrap for a fresh Google-only OAuth signup. Linked or established KLEIO account roles are immutable.';
