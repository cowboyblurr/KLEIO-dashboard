...
alter table public.artist_import_sources
  ...
  check (source_type = any (array[
    ...
    'google_drive_video'::text,
    'google_drive_audio'::text,
...
values
  ...
  ('google_drive_video', false, 'Google Drive', 'Deferred. Video selection through Google Drive remains disabled until the connected-provider flow is validated.'),
  ('google_drive_audio', false, 'Google Drive', 'Deferred. Audio selection through Google Drive remains disabled until the connected-provider flow is validated.')
on conflict (source_type) do update set
  artist_beta_enabled = excluded.artist_beta_enabled,
  artist_label = excluded.artist_label,
  availability_note = excluded.availability_note,
  updated_at = now();

update public.kleio_import_source_availability
set artist_label = 'Device upload',
    availability_note = 'Active beta path for artist-controlled supporting-document upload to the private KLEIO Media Library.',
    updated_at = now()
where source_type = 'device_document';

update public.kleio_import_source_availability
set artist_label = 'Device upload',
    availability_note = 'Active beta path for artist-controlled image upload to the private KLEIO Media Library.',
    updated_at = now()
where source_type = 'device_image';

update public.kleio_import_source_availability
set artist_label = 'PDF analysis',
    availability_note = 'Active optional path for private PDF document analysis; PDF is not the only upload format.',
    updated_at = now()
