-- Prefer the full interpreted intent when the catalog can satisfy it.
-- When no exact structured combination exists, fall back to the verified primary
-- practice so conversational searches do not end in a false zero-result state.

insert into public.opportunity_search_stop_terms(normalized_term, language_code)
values ('am','en'), ('accepting','en'), ('work','en')
on conflict do nothing;

with additions(category, canonical_value, alias, language_code, relation_type, match_weight, display_priority) as (
  values
    ('discipline','Film','filmmaker','en','synonym',1.15,6),
    ('discipline','Film','filmmakers','en','synonym',1.15,7),
    ('fee','no_fee','free','en','synonym',0.90,3)
)
...
as $function$
  ...
  raw_candidates as (
    select
      ...
      (
        ...
        where exists (
          select 1
          from unnest(coalesce(opportunity_row.disciplines, '{}'::text[])) d
          where public.normalize_opportunity_search_text(d) =
                public.normalize_opportunity_search_text(query_discipline)
        ) or exists (
          select 1
        ...
        where exists (
          ...
          where public.normalize_opportunity_search_text(location_value) =
             ...
             or (
               ...
               and public.normalize_opportunity_search_text(location_value) in ('worldwide','global','international')
             )
        )
  ...
  availability as (
    select coalesce(bool_or(exact_structured_match), false) as has_exact_structured_match
    from candidates
  ),
  ranked as (
    select
      candidate.*,
      availability.has_exact_structured_match,
      (
