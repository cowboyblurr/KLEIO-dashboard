-- Generic labels such as `International` or `Cross-border` do not prove that
-- every nationality may apply. Country-specific fallback requires explicit
-- Worldwide, Global, All countries, or All nationalities eligibility.
    ...
    select
        ...
        where exists (
          ...
          where public.normalize_opportunity_search_text(location_value) = public.normalize_opportunity_search_text(query_location)
             ...
             or (
               ...
               and public.normalize_opportunity_search_text(location_value) in ('worldwide','global','all countries','all nationalities')
             )
        )
      ...
      exists (
        ...
        where public.normalize_opportunity_search_text(eligibility_value) in (
          'worldwide','global','all countries','all nationalities'
        )
