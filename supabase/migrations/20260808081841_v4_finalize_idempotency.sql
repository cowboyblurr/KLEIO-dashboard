create unique index if not exists application_submission_versions_one_per_package_version
  on public.application_submission_versions(package_id, source_package_version)
  where source_package_version is not null;
...
begin
  ...
  if package_row.id is null then raise exception 'application_package_not_found'; end if;
  if package_row.artist_approved_at is null then raise exception 'artist_approval_required'; end if;
  if package_row.state in ('draft', 'missing_information', 'deadline_passed', 'failed', 'withdrawn') then raise exception 'application_package_not_finalizable'; end if;
  if jsonb_typeof(supplied_preflight) <> 'object' then raise exception 'invalid_preflight_snapshot'; end if;
  if coalesce((supplied_preflight->>'blocking_count')::integer, 0) > 0 then raise exception 'preflight_blockers_remaining'; end if;
  ...
  from (
    select opportunity.id, opportunity.title, opportunity.provider_name, opportunity.deadline_at,
           opportunity.deadline_timezone, opportunity.submission_method, opportunity.submission_email,
           opportunity.submission_instructions, opportunity.canonical_url, opportunity.application_url,
