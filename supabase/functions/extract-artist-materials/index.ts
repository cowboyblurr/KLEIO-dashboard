...
async function analyze(admin: AdminClient, userId: string, body: JsonObject) {
  ...
  try {
    ...
    let quality = assessDocumentCoverage({ classification, totalPages, pagesAnalyzed: analysis.document_assessment.pages_analyzed, unreadablePages: analysis.document_assessment.unreadable_pages, claims: analysis.claims, sections: analysis.sections, providerAvailable: true, textQuality: analysis.document_assessment.text_quality })
    ...
    if (escalate) try {
      ...
      const next = validateDocumentAnalysis(pro.output, { totalPages, nativePages: pages, requestedClassification: classification }), nextQuality = assessDocumentCoverage({ classification, totalPages, pagesAnalyzed: next.document_assessment.pages_analyzed, unreadablePages: next.document_assessment.unreadable_pages, claims: next.claims, sections: next.sections, providerAvailable: true, textQuality: next.document_assessment.text_quality })
