...
type SourceRow = { id: string; slug: string; name: string; base_domain: string }
type OpportunityRow = { id: string; title: string; provider_name: string; canonical_url: string; preview_image_origin: string; preview_image_url: string }
...
function extractCandidates(html: string, pageUrl: URL, title: string) {
  ...
  for (const tag of html.match(/<meta\b[^>]*>/gi) ?? []) {
    ...
    const resolved = safeUrl(attribute(tag, "content"), pageUrl.toString())
  ...
  for (const tag of html.match(/<img\b[^>]*>/gi) ?? []) {
    const resolved = safeUrl(
      attribute(tag, "data-src") || attribute(tag, "data-lazy-src") || attribute(tag, "src"),
      pageUrl.toString(),
    )
    if (!resolved) continue
    const candidate = { url: resolved.toString(), alt: attribute(tag, "alt"), score: 0 }
...
async function validImage(candidate: URL, baseDomain: string) {
  if (!hostMatches(candidate.hostname, baseDomain) || GENERIC_IMAGE_PATTERN.test(candidate.pathname)) return false
  const controller = new AbortController()
...
async function enrichSource(supabase: SupabaseClient, source: SourceRow) {
  ...
  for (const opportunity of (data ?? []) as OpportunityRow[]) {
    ...
    try {
      ...
      for (const candidate of extractCandidates(html, pageUrl, opportunity.title)) {
