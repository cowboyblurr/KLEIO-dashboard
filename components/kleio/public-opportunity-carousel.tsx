...
import { ArrowLeft, ArrowRight, CalendarDays, Loader2, MapPin, Pause, Play, RotateCcw } from "lucide-react"
...
function fundingLabel(item: PublicOpportunityCarouselItem) {
  ...
  if (item.award_min === null && item.award_max === null) return "Funding not stated by the source"
...
export function PublicOpportunityCarousel() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: requestError } = await supabase.rpc("get_public_opportunity_carousel", { limit_count: 8 })
      if (requestError) throw requestError
      const next = (data ?? []) as PublicOpportunityCarouselItem[]
      setItems(next)
      if (next.length) void trackKleioProductEvent("carousel_viewed", { surface: "landing", metadata: { result_count: next.length } })
    } catch (reason) {
  ...
  useEffect(() => {
    if (reducedMotion || paused || interactionPaused || !tabVisible || items.length < 2) return
    const timer = window.setInterval(() => advance(1), 9000)
    return () => window.clearInterval(timer)
  }, [advance, interactionPaused, items.length, paused, reducedMotion, tabVisible])

  return (
    <section className="border-y border-[#E9E3F3] bg-[#FCFBFE] py-14 sm:py-16" aria-labelledby="recent-opportunities-title">
      <div className="mx-auto max-w-[1280px] px-5 sm:px-8">
        <div className="flex flex-wrap items-end justify-between gap-5">
          <div className="max-w-2xl">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#75639E]">Public opportunity discovery</p>
            <h2 id="recent-opportunities-title" className="mt-2 font-serif text-3xl tracking-[-0.035em] text-[#292631] sm:text-4xl">Recently added opportunities</h2>
            <p className="mt-3 text-sm leading-6 text-[#746E80]">Grants, residencies, exhibitions, commissions, awards, and open calls from sourced listings.</p>
          </div>
        ...
        {loading && (
          <div className="mt-8 grid min-h-64 place-items-center rounded-3xl border border-[#E7E1F7] bg-white" role="status">
            <p className="flex items-center gap-2 text-sm text-[#746E80]"><Loader2 className="size-4 animate-spin" />Loading sourced opportunities…</p>
          </div>
        )}

        {error && (
          <div className="mt-8 rounded-3xl border border-[#E7E1F7] bg-white p-8" role="alert">
            <p className="font-serif text-xl text-[#292631]">Recent opportunities are temporarily unavailable.</p>
            <p className="mt-2 text-sm text-[#746E80]">The rest of KLEIO remains available. You can open the full directory or try this preview again.</p>
            <div className="mt-5 flex flex-wrap gap-3"><button type="button" onClick={() => void load()} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-[#D8D0F2] px-4 text-sm font-semibold text-[#5B4B8A]"><RotateCcw className="size-4" />Retry</button><Link href="/opportunities/" className="inline-flex min-h-10 items-center rounded-xl bg-[#5B4B8A] px-4 text-sm font-semibold text-white">Open full directory</Link></div>
          </div>
        )}
        ...
        {!loading && !error && items.length === 0 && (
          <div className="mt-8 rounded-3xl border border-[#E7E1F7] bg-white p-8">
            ...
            <p className="mt-2 text-sm text-[#746E80]">KLEIO only displays records that pass the public visibility and source-quality rules.</p>
            <Link href="/opportunities/" className="mt-5 inline-flex min-h-10 items-center rounded-xl bg-[#5B4B8A] px-4 text-sm font-semibold text-white">Browse the full directory</Link>
          </div>
        )}
        ...
        {!loading && !error && items.length > 0 && (
          <div
            ...
            aria-label="Recently added artist opportunities"
          ...
          >
            {items.map((item, index) => {
              ...
              const detailHref = `/opportunities/?opportunity=${encodeURIComponent(item.id)}&source=landing_carousel`
              return (
                <article key={item.id} data-opportunity-card className="relative min-w-[84vw] snap-start overflow-hidden rounded-[1.4rem] border border-[#E4DDF1] bg-white shadow-[0_22px_60px_rgba(70,52,112,0.08)] sm:min-w-[390px] lg:min-w-[calc((100%-32px)/3)]" role="group" aria-roledescription="slide" aria-label={`${index + 1} of ${items.length}: ${item.title}`}>
                  ...
                  <div className="p-5">
                    ...
                    <p className="mt-3 line-clamp-2 min-h-10 text-sm leading-5 text-[#746E80]">{item.summary || "Review the sourced listing for details."}</p>
                    ...
                    <Link href={detailHref} onClick={() => void trackKleioProductEvent("carousel_card_selected", { surface: "landing", opportunityId: item.id, metadata: { source: "landing_carousel" } })} className="mt-5 inline-flex min-h-10 w-full items-center justify-center rounded-xl border border-[#CFC3ED] bg-white text-sm font-semibold text-[#5B4B8A] transition-colors hover:bg-[#F7F4FF] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#A997E8]/20">View opportunity</Link>
                  </div>
                </article>
              )
            })}
        ...
        <div className="mt-6 flex justify-center"><Link href="/opportunities/" className="inline-flex min-h-11 items-center rounded-xl bg-[#5B4B8A] px-5 text-sm font-semibold text-white">Explore all opportunities<ArrowRight className="ml-2 size-4" /></Link></div>
      </div>
    </section>
  )
}
