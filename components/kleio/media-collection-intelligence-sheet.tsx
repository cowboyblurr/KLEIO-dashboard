...
import { Check, Layers3, Loader2, ShieldCheck, X } from "lucide-react"
...
const synthesisStages = [
  "Comparing recurring terms and themes across the selected sources…",
  "Organizing formal, material, spatial, and sonic relationships…",
  "Noting where works repeat, contrast, or extend one another…",
  "Keeping source-supported patterns separate from optional interpretation…",
  "Preparing editable body-of-work language for your review…",
]
...
export function MediaCollectionIntelligenceSheet({ items, open, initialInsight = null, onClose, onConfirmed, onDismissed }: Props) {
      ...
      try {
        ...
        if (missing.length) {
          ...
          for (let index = 0; index < missing.length; index += 1) {
            ...
            setStatus(`Preparing ${item.title} · ${index + 1} of ${missing.length}`)
        ...
        if (!cancelled) {
          setPhase("idle")
          setError(reason instanceof Error ? reason.message : "Media Assist could not compare these private sources.")
  ...
  async function confirm() {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : "Media Assist could not save your reviewed note.")
  ...
  async function dismiss() {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : "Media Assist could not dismiss these suggestions.")
    } finally { setSaving(false) }
  }

  return <div className="fixed inset-0 z-[125] flex justify-end bg-[#21192D]/35 backdrop-blur-[2px]">
    <button type="button" className="absolute inset-0" aria-label="Close Media Assist comparison" onClick={onClose} />
    <aside role="dialog" aria-modal="true" aria-labelledby="media-assist-comparison-title" aria-busy={phase === "individual" || phase === "synthesis"} className="relative z-10 flex h-dvh w-full max-w-[680px] flex-col overflow-hidden border-l border-[#DDD5EE] bg-[#FCFBFE] text-[#292631] shadow-[-24px_0_70px_rgba(54,42,82,0.16)]">
      <header className="flex shrink-0 items-start justify-between gap-4 border-b border-[#E7E1F7] bg-white px-5 py-4"><div><p className="text-[0.66rem] font-semibold uppercase tracking-[0.16em] text-[#75639E]">Media Assist · private comparison</p><h2 id="media-assist-comparison-title" className="mt-1 font-serif text-2xl font-semibold">Compare selected media</h2><p className="mt-1 text-xs leading-5 text-[#746E80]">{items.length || insight?.sourceIds.length || 0} selected sources · suggestions stay private until you decide what is useful.</p></div><button type="button" onClick={onClose} className="grid size-10 place-items-center rounded-xl border border-[#E2DCF1] bg-white" aria-label="Close"><X className="size-4" /></button></header>
      <div className="min-h-0 flex-1 overflow-y-auto p-5">
        ...
        {(phase === "individual" || phase === "synthesis") && <section className="rounded-[22px] border border-[#E2DCF1] bg-white p-5" aria-live="polite"><div className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-[#F0EAFB] text-[#5B4B8A]"><Layers3 className="size-5" /></span><div><h3 className="font-serif text-xl font-semibold">Preparing the selected work together</h3><p className="mt-2 text-sm leading-6 text-[#625C70]">Media Assist first prepares any missing private source notes, then organizes possible relationships across the group. It does not rank the work or decide what the work means.</p></div></div><div className="mt-5 rounded-2xl border border-[#E7E1F7] bg-[#FAF8FE] p-4"><p className="text-[0.68rem] font-semibold uppercase tracking-[0.14em] text-[#75639E]">Media Assist is preparing</p>{phase === "individual" ? <div className="mt-3 flex items-center gap-3 text-sm font-semibold text-[#4F407B]"><Loader2 className="size-4 animate-spin" />{status || "Preparing private source notes…"}</div> : <div className="mt-3 space-y-2">{synthesisStages.map((label, index) => <div key={label} className={`flex items-center gap-2 text-sm ${index === stage ? "font-semibold text-[#4F407B]" : index < stage ? "text-[#746E80]" : "text-[#A39CAB]"}`}>{index < stage ? <Check className="size-3.5" /> : index === stage ? <Loader2 className="size-3.5 animate-spin" /> : <span className="size-3.5" />}{label}</div>)}</div>}</div><p className="mt-3 text-xs leading-5 text-[#8A8296]">No creative score or confidence percentage is shown. Keep only the language that feels accurate to your practice.</p></section>}

        {phase === "idle" && !insight && !error && <section className="rounded-[22px] border border-[#E2DCF1] bg-white p-5 text-sm leading-6 text-[#746E80]">Select at least two supported private sources in Media Library, then choose <strong className="text-[#5B4B8A]">Run Media Assist</strong>.</section>}

        {insight && phase === "ready" && <div className="space-y-4">
          <section className="rounded-[22px] border border-[#E2DCF1] bg-white p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[0.68rem] font-semibold uppercase tracking-[0.14em] text-[#75639E]">Editable suggestions</p><h3 className="mt-1 font-serif text-2xl font-semibold">{insight.title}</h3></div><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${insight.status === "confirmed" ? "bg-emerald-50 text-emerald-800" : "bg-[#F2EEFA] text-[#665A85]"}`}>{insight.status === "confirmed" ? "Artist-approved context" : "Needs your review"}</span></div><p className="mt-3 text-sm leading-7 text-[#4E4954]">{insight.summary || "Media Assist found possible relationships across the selected sources. Review the notes below and keep only what reflects your practice."}</p></section>

          <PatternList title="Possible recurring themes" patterns={insight.recurringThemes} sourceLabel={sourceLabel} />
          ...
          <PatternList title="Possible dialogue between works" patterns={insight.workDialogues} sourceLabel={sourceLabel} />
