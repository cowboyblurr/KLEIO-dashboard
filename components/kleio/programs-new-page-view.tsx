      ...
      <div className="mx-auto min-w-[760px] max-w-6xl">
        <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <h1 className="mt-2 font-serif text-3xl font-semibold tracking-tight text-foreground">Create open call</h1>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted-foreground">Build the institutional entry point for {institution.name}: opportunity details, required materials, application questions, rubric, and committee coverage before submissions arrive.</p>
          </div>
          <span className="rounded-full border border-border bg-card px-3 py-1.5 text-xs font-medium text-muted-foreground">Working demo workflow</span>
        </div>
        ...
        {published && (
          <div className="mb-4 rounded-xl border border-[oklch(0.85_0.07_150)] bg-[oklch(0.96_0.04_150)] px-3 py-3 text-sm text-[oklch(0.4_0.12_150)]">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <Link href="/review-queue/" className="inline-flex h-9 items-center rounded-xl bg-primary px-3 text-xs font-semibold text-primary-foreground transition-colors hover:bg-primary/90">Open Review Queue</Link>
              <Link href="/review-room/" className="inline-flex h-9 items-center rounded-xl border border-[oklch(0.85_0.07_150)] bg-white px-3 text-xs font-semibold text-[oklch(0.4_0.12_150)] transition-colors hover:bg-white/70">Enter Review Room</Link>
            </div>
          </div>
        )}
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]">
          ...
          <section className="rounded-2xl border border-border bg-card p-5 shadow-sm kleio-card-shadow">
            ...
            <div className="mt-4 space-y-3"><Info label="Application deadline" value={program.deadline} /><Info label="Review period starts" value={program.reviewStart} /><Info label="Decision date" value={program.decisionDate} /></div>
          </section>
          ...
          <section className="rounded-2xl border border-border bg-card p-5 shadow-sm kleio-card-shadow">
            ...
            <div className="mt-4 space-y-2">{applicationQuestions.map((item, index) => <div key={item} className={`${workflowMotion.step} rounded-xl border border-border bg-background px-3 py-2 text-sm text-foreground`} style={workflowDelay(index)}>{item}</div>)}</div>
          </section>
          ...
          <section className="rounded-2xl border border-border bg-card p-5 shadow-sm kleio-card-shadow">
            ...
            <div className="mt-4 space-y-3">
              {committee.map((person, index) => {
                ...
                return <div key={person.id} className={`${workflowMotion.step} flex items-center justify-between rounded-xl border border-border bg-background p-3`} style={workflowDelay(index)}><div><p className="text-sm font-medium text-foreground">{person.name}</p><p className="text-xs text-muted-foreground">{person.role} · {person.inviteStatus}</p></div><span className="rounded-full bg-accent px-2 py-1 text-xs font-medium text-accent-foreground">{progress ? `${progress.completed}/${progress.assigned}` : "0/0"} reviews</span></div>
              })}
            </div>
          </section>
        </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <button type="button" onClick={() => setPublished(true)} className="inline-flex h-11 items-center rounded-xl bg-primary px-5 text-sm font-semibold text-primary-foreground shadow-sm transition-colors hover:bg-primary/90">Publish open call</button>
          <Link href="/programs/" className="inline-flex h-11 items-center rounded-xl border border-border bg-card px-5 text-sm font-semibold text-foreground transition-colors hover:bg-accent/50">Back to Programs</Link>
          <Link href="/review-queue/" className="inline-flex h-11 items-center rounded-xl border border-border bg-card px-5 text-sm font-semibold text-foreground transition-colors hover:bg-accent/50">Review Queue</Link>
        </div>
      </div>
    </main>
  )
