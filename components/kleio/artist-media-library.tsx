...
export function ArtistMediaLibrary() {
  ...
  async function analyze(item: ArtistMediaLibraryItem) {
    ...
    setActiveId(item.id); setError(""); setMessage("")
    try { const result = await requestMediaExtraction(item); setMessage(result.warnings.includes("ocr_required") ? "The PDF is private, but it needs OCR or manual classification before its contents can become Passport updates." : `${result.proposalCount} reviewable Passport update${result.proposalCount === 1 ? "" : "s"} prepared.`) }
    catch (reason) { setError(reason instanceof Error ? reason.message : "KLEIO could not analyze this private PDF.") }
    finally { setActiveId("") }
  }
  ...
  return <main className="h-full overflow-y-auto bg-[#FCFBFE] px-4 py-5 sm:px-6 sm:py-6"><div className="mx-auto max-w-[1240px] space-y-5">
    ...
    <section className="rounded-[26px] border border-[#E2DCF1] bg-[linear-gradient(135deg,#F8F5FF,#FFFFFF)] p-5 shadow-[0_18px_52px_rgba(82,64,130,0.06)]"><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div className="max-w-2xl"><p className="text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-[#75639E]">Private reusable sources</p><h2 className="mt-2 font-serif text-2xl font-semibold tracking-[-0.03em]">One private library for the material you bring into KLEIO</h2><p className="mt-2 text-sm leading-6 text-[#746E80]">Upload common images, video, audio, and documents from your device. Files remain private until you explicitly reuse them. PDF analysis is an optional separate workflow—not the definition of what KLEIO can store.</p></div><div className="flex flex-wrap gap-2"><QuickMediaImport context="existing_media_library" label="Upload media" onConfirm={async () => { await refresh(); setMessage("Your private media is available in the library.") }} /><Link href="/artist-dashboard/import/" className={secondary}><FileSearch className="size-4" />Import & analyze</Link></div></div><p className="mt-4 text-xs font-semibold text-[#6A5896]"><ShieldCheck className="mr-1.5 inline size-3.5" />Institutions cannot browse this library, extraction jobs, rejected proposals, or unconfirmed interpretations. They see only material you explicitly authorize.</p></section>
