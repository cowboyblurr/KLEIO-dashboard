...
/* eslint-disable @next/next/no-img-element -- private library images use short-lived signed URLs */

import Link from "next/link"
import { useEffect, useMemo, useState } from "react"
import { ArrowRight, FileSearch, FileText, ImageIcon, Library, Loader2, Search, ShieldCheck, Sparkles, Trash2, UploadCloud } from "lucide-react"
...
export function ArtistMediaLibrary() {
  ...
  const refresh = () => loadArtistMediaLibrary({ limit: 200 }).then(setItems)
  ...
  async function analyze(item: ArtistMediaLibraryItem) {
    ...
    try {
      ...
      setMessage(result.warnings.includes("ocr_required")
        ? "The source is safe, but this PDF needs OCR or manual classification before its contents can become Passport updates."
        : `${result.proposalCount} reviewable Passport update${result.proposalCount === 1 ? "" : "s"} prepared.`)
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "KLEIO could not analyze this private source.")
    } finally {
      setActiveId("")
    }
  }
  ...
  return (
    <main className="h-full overflow-y-auto bg-[#FCFBFE] px-4 py-5 sm:px-6 sm:py-6">
      <div className="mx-auto max-w-[1240px] space-y-5">
        <WorkspacePageHeader eyebrow="Artist workspace" title="Media Library" description="Keep private images and documents in one place, then review and reuse artist-approved material across your Creative Passport and applications." />
        <section className="rounded-[26px] border border-[#E2DCF1] bg-[linear-gradient(135deg,#F8F5FF,#FFFFFF)] p-5 shadow-[0_18px_52px_rgba(82,64,130,0.06)]">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl"><p className="text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-[#75639E]">Private reusable sources</p><h2 className="mt-2 font-serif text-2xl font-semibold tracking-[-0.03em]">One library for the work you bring into KLEIO</h2><p className="mt-2 text-sm leading-6 text-[#746E80]">Use the guided import flow for artwork, website material, or Google Drive. Use quick upload when you only need to add a single private image or document. Nothing becomes public without your approval.</p></div>
            <div className="flex flex-wrap gap-2">
              <Link href="/artist-dashboard/import/" className={primary}><UploadCloud className="size-4" />Import work <ArrowRight className="size-4" /></Link>
              <QuickMediaImport
                context="creative_passport"
                label="Quick upload"
                onConfirm={async ({ items: imported }) => {
                  const documents = imported.filter((item) => item.mediaKind === "document" && item.sourceId)
                  for (const item of documents) await requestMediaExtraction(item)
                  await refresh()
                  setMessage(documents.length ? "Media added privately and document analysis started." : "Media added to your private KLEIO Library.")
                }}
              />
              <Link href="/artist-dashboard/passport/review/" className={secondary}><Sparkles className="size-4" />Review Passport updates</Link>
            </div>
          </div>
          <p className="mt-4 text-xs font-semibold text-[#6A5896]"><ShieldCheck className="mr-1.5 inline size-3.5" />Institutions cannot browse this library or its extracted claims. They see only material you authorize for a profile or application.</p>
        </section>
        ...
        <section className="rounded-[22px] border border-[#E7E1F7] bg-white p-4 shadow-[0_14px_42px_rgba(82,64,130,0.05)]">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><label className="relative block flex-1 lg:max-w-md"><span className="sr-only">Search private media</span><Search className="pointer-events-none absolute left-3 top-3.5 size-4 text-[#8A8296]" /><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search artwork, filename, or document" className="h-11 w-full rounded-xl border border-[#DED7EF] bg-white pl-9 pr-3 text-sm outline-none focus:border-[#A997E8] focus:ring-4 focus:ring-[#A997E8]/12" /></label><div className="flex flex-wrap gap-2" aria-label="Media filters">{(["all","image","document","approved","unattached"] as const).map((value) => <button key={value} type="button" aria-pressed={filter === value} onClick={() => setFilter(value)} className={`${filterButton} ${filter === value ? "border-[#A997E8] bg-[#F1ECFB] text-[#5B4B8A]" : "border-[#E7E1F7] bg-white text-[#746E80]"}`}>{value === "all" ? "All media" : value === "image" ? "Artwork & images" : value === "document" ? "Documents" : value === "approved" ? "Approved works" : "Unattached"}</button>)}</div></div>
        </section>

        {loading ? <div className="flex items-center justify-center rounded-[22px] border border-[#E7E1F7] bg-white p-10 text-sm text-[#746E80]"><Loader2 className="mr-2 size-4 animate-spin" />Loading your private media library…</div> : visible.length ? <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{visible.map((item) => <article key={item.id} className="group overflow-hidden rounded-[22px] border border-[#E7E1F7] bg-white shadow-[0_14px_38px_rgba(82,64,130,0.05)]"><div className="relative grid aspect-[4/3] place-items-center overflow-hidden bg-[#F4F1F8]">{item.previewUrl ? <img src={item.previewUrl} alt="" className="size-full object-cover transition duration-500 group-hover:scale-[1.02]" loading="lazy" /> : item.mediaKind === "document" ? <FileText className="size-9 text-[#75639E]" /> : <ImageIcon className="size-9 text-[#75639E]" />}<span className="absolute left-2 top-2 rounded-full bg-white/95 px-2.5 py-1 text-[0.65rem] font-semibold text-[#5B4B8A]">{item.approvalState === "approved" ? "Approved work" : "Private source"}</span></div><div className="p-4"><h3 className="truncate font-serif text-lg font-semibold">{item.title}</h3><p className="mt-1 truncate text-xs text-[#746E80]">{item.originalFilename}</p><dl className="mt-3 grid grid-cols-2 gap-2 text-[0.68rem]"><div><dt className="font-semibold uppercase tracking-wide text-[#8A8296]">Source</dt><dd className="mt-0.5 truncate text-[#625C70]">{item.sourceType.replaceAll("_", " ")}</dd></div><div><dt className="font-semibold uppercase tracking-wide text-[#8A8296]">Uses</dt><dd className="mt-0.5 text-[#625C70]">{Math.max(item.usageCount, item.associatedWorkId ? 1 : 0)}</dd></div></dl>{item.associatedWorkTitle && <p className="mt-3 rounded-lg bg-[#F7F4FF] px-2.5 py-2 text-xs font-semibold text-[#5B4B8A]">Artwork: {item.associatedWorkTitle}</p>}<div className="mt-3 flex items-center justify-between gap-2"><span className="text-[0.68rem] text-[#8A8296]">Added {new Date(item.createdAt).toLocaleDateString()}</span><span className="flex items-center gap-1">{item.mediaKind === "document" && item.sourceId && <button type="button" onClick={() => void analyze(item)} disabled={Boolean(activeId)} className="grid size-9 place-items-center rounded-lg text-[#6A5896] hover:bg-[#F7F4FF] disabled:opacity-50" aria-label={`Analyze ${item.title} for Passport updates`}>{activeId === item.id ? <Loader2 className="size-4 animate-spin" /> : <FileSearch className="size-4" />}</button>}{item.usageCount === 0 && !item.associatedWorkId && <button type="button" onClick={() => void archive(item)} className="grid size-9 place-items-center rounded-lg text-[#8A8296] hover:bg-red-50 hover:text-red-700" aria-label={`Archive ${item.title}`}><Trash2 className="size-4" /></button>}</span></div></div></article>)}</div> : <div className="grid place-items-center rounded-[22px] border border-dashed border-[#D8D0F2] bg-white p-12 text-center"><span className="grid size-14 place-items-center rounded-2xl bg-[#F0EAFB] text-[#5B4B8A]"><Library className="size-6" /></span><h2 className="mt-4 font-serif text-2xl font-semibold">No matching media</h2><p className="mt-2 max-w-md text-sm leading-6 text-[#746E80]">Import work, add a private source, or choose another filter. KLEIO will preserve each file without making it public.</p><Link href="/artist-dashboard/import/" className={`${primary} mt-5`}><UploadCloud className="size-4" />Import work</Link></div>}
      </div>
    </main>
  )
}
