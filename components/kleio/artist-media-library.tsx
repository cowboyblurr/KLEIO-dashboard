...
import { AudioLines, BrainCircuit, Check, FileText, ImageIcon, Layers3, Library, Loader2, Search, ShieldCheck, Sparkles, Video, X } from "lucide-react"
...
export function ArtistMediaLibrary() {
  ...
  function openCollectionAnalysis() {
    if (selectedItems.length < 2) { setError("Select at least two analyzable private sources to compare together."); return }
    setError("")
  ...
  return <main className="h-full overflow-y-auto bg-[#FCFBFE] px-4 py-5 sm:px-6 sm:py-6"><div className="mx-auto max-w-[1240px] space-y-5">
    ...
    <section className="rounded-[26px] border border-[#E2DCF1] bg-[linear-gradient(135deg,#F8F5FF,#FFFFFF)] p-5 shadow-[0_18px_52px_rgba(82,64,130,0.06)]"><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div className="max-w-2xl"><p className="text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-[#75639E]">Private reusable sources</p><h2 className="mt-2 font-serif text-2xl font-semibold tracking-[-0.03em]">Upload, understand, and reuse your material in one place</h2><p className="mt-2 text-sm leading-6 text-[#746E80]">Upload supported images, video, audio, and documents. Analyze one source on its own or select several to understand recurring themes, relationships, and body-of-work language across the group.</p></div><QuickMediaImport context="existing_media_library" label="Upload media" onConfirm={async ({ items: uploaded }) => { const next = await refresh(); const analyzableIds = uploaded.flatMap((item) => item.sourceId && canAnalyzeMediaItem(item) ? [item.sourceId] : []); if (analyzableIds.length > 1) { setSelectionMode(true); setSelectedIds(analyzableIds.slice(0, 12)); setMessage(`${analyzableIds.length} uploaded sources are selected. Analyze them together when you are ready.`) } else { setMessage("Your private media is available in the library.") } setItems(next) }} /></div><p className="mt-4 text-xs font-semibold text-[#6A5896]"><ShieldCheck className="mr-1.5 inline size-3.5" />Institutions cannot browse this library or private analysis. Group patterns become reusable application context only after you review and keep the final note.</p></section>

    {collectionInsights.length > 0 && <section className="rounded-[24px] border border-[#E1DAF0] bg-white p-4 shadow-[0_14px_42px_rgba(82,64,130,0.04)]" aria-labelledby="body-of-work-intelligence-heading"><div className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-[#F0EAFB] text-[#5B4B8A]"><Layers3 className="size-5" /></span><div><h2 id="body-of-work-intelligence-heading" className="text-sm font-semibold text-[#292631]">Body-of-work intelligence</h2><p className="mt-1 text-xs leading-5 text-[#746E80]">Recent group analyses stay private here. Only artist-approved notes can support future application drafts.</p></div></div><div className="mt-4 grid gap-3 lg:grid-cols-3">{collectionInsights.slice(0, 3).map((insight) => <article key={insight.id} className="rounded-2xl border border-[#E7E1F7] bg-[#FCFBFE] p-4"><div className="flex items-start justify-between gap-2"><h3 className="font-serif text-lg font-semibold text-[#292631]">{insight.title}</h3><span className={`shrink-0 rounded-full px-2 py-1 text-[0.65rem] font-semibold ${insight.status === "confirmed" ? "bg-emerald-50 text-emerald-800" : "bg-[#F1ECFB] text-[#665A85]"}`}>{insight.status === "confirmed" ? "Artist-approved" : "Review"}</span></div><p className="mt-2 line-clamp-3 text-xs leading-5 text-[#746E80]">{insight.artistSummary || insight.bodyOfWorkSummary || insight.summary}</p><button type="button" className={`${secondary} mt-3`} onClick={() => reviewCollection(insight)}>{insight.status === "confirmed" ? "Open context" : "Review analysis"}</button></article>)}</div></section>}
