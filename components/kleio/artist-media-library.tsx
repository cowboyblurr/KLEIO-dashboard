  ...
  return (
    <main className="h-full overflow-y-auto bg-[#FCFBFE] px-4 py-5 sm:px-6 sm:py-6">
      <div className="mx-auto max-w-[1240px] space-y-5">
        ...
        <section className="rounded-[26px] border border-[#E2DCF1] bg-[linear-gradient(135deg,#F8F5FF,#FFFFFF)] p-5 shadow-[0_18px_52px_rgba(82,64,130,0.06)]">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl">
              ...
              <h2 className="mt-2 font-serif text-2xl font-semibold tracking-[-0.03em]">One private library for material you bring into KLEIO</h2>
              <p className="mt-2 text-sm leading-6 text-[#746E80]">The initial document beta accepts direct PDF uploads. Files remain private until you separately approve a Passport record, Portfolio use, profile use, or application attachment. Connected providers are deferred.</p>
            </div>
            <Link href="/artist-dashboard/import/" className={primary}><UploadCloud className="size-4" />Upload PDF document <ArrowRight className="size-4" /></Link>
          </div>
        ...
        </article>)}</div> : <div className="grid place-items-center rounded-[22px] border border-dashed border-[#D8D0F2] bg-white p-12 text-center"><span className="grid size-14 place-items-center rounded-2xl bg-[#F0EAFB] text-[#5B4B8A]"><Library className="size-6" /></span><h2 className="mt-4 font-serif text-2xl font-semibold">No matching media</h2><p className="mt-2 max-w-md text-sm leading-6 text-[#746E80]">Choose another filter or upload a private PDF document. KLEIO preserves every source and waits for your approval before reusing it.</p><Link href="/artist-dashboard/import/" className={`${primary} mt-5`}><UploadCloud className="size-4" />Upload PDF document</Link></div>}
      </div>
    </main>
  )
}
