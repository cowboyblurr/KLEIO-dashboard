...
const menuItem = "flex min-h-10 w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs font-semibold text-[#4F485A] transition hover:bg-[#F6F3FA] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#A997E8]/20"
...
const copy = {
  en: {
    ...
    delete: "Delete from KLEIO",
    title: "Delete from KLEIO?",
    ...
    cancel: "Cancel",
    close: "Close",
    deleteNow: "Delete from KLEIO",
    detachDelete: "Remove references and delete",
  ...
  es: {
    more: "Más acciones",
    preview: "Vista previa / abrir",
    remove: "Quitar de esta sección",
    delete: "Eliminar de KLEIO",
    title: "¿Eliminar de KLEIO?",
    ...
    cancel: "Cancelar",
    close: "Cerrar",
    deleteNow: "Eliminar de KLEIO",
    detachDelete: "Quitar referencias y eliminar",
...
function referenceSummary(assessment: MediaDeletionAssessment | null) {
  ...
  return [
    ...assessment.finalizedReferences,
    ...assessment.blockingReferences,
    ...assessment.editableReferences,
  ].slice(0, 6)
}
...
export function MediaManagementMenu({ item, currentContext, onChanged }: Props) {
  ...
  useEffect(() => {
    setLanguage(document.documentElement.lang.toLowerCase().startsWith("es") ? "es" : "en")
  }, [])
  ...
  async function detach() {
    ...
    try {
      await detachMediaFromContext(item, currentContext)
      setMessage(t.detachSuccess)
      setOpen(false)
      await onChanged?.()
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "KLEIO could not remove this media from the current section.")
    } finally {
      setBusy(false)
    }
  }
  ...
  async function prepareDelete() {
    setBusy(true); setError(""); setMessage("")
    try {
      setAssessment(await loadMediaDeletionAssessment(item))
      setConfirmOpen(true)
      setOpen(false)
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "KLEIO could not check where this media is being used.")
    } finally {
      setBusy(false)
    }
  }
  ...
  async function remove() {
    ...
    try {
      ...
      setError(reason instanceof Error ? reason.message : "KLEIO could not safely delete this media.")
    } finally {
      setBusy(false)
    }
  }
  ...
  return <>
    <div ref={rootRef} className="relative">
      ...
      {open && <div role="menu" className="absolute right-0 top-10 z-40 w-56 rounded-xl border border-[#E1DAF0] bg-white p-1.5 shadow-[0_18px_48px_rgba(54,42,82,0.16)]">
        {item.previewUrl && <button type="button" role="menuitem" className={menuItem} onClick={() => { window.open(item.previewUrl || "", "_blank", "noopener,noreferrer"); setOpen(false) }}><ExternalLink className="size-4" />{t.preview}</button>}
