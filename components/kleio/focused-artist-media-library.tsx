...
import { FileSearch, FileText, ImageIcon, Library, Loader2, Search, ShieldCheck, Trash2 } from "lucide-react"
...
export function FocusedArtistMediaLibrary() {
  ...
  async function analyze(item: ArtistMediaLibraryItem) {
    ...
    try {
      ...
      setMessage(result.warnings.includes("ocr_required")
        ? "The source is private, but this PDF needs OCR or manual classification before its contents can become Passport updates."
    ...
    <main className="h-full overflow-y-auto bg-[#FCFBFE] px-4 py-5 sm:px-6 sm:py-6">
      <div className="mx-auto max-w-[1240px] space-y-5">
        <WorkspacePageHeader
          ...
          title="Media Library"
          description="Find and reuse private images and documents across your Portfolio, Creative Passport, profile, and applications."
          primaryCta={{ label: "Upload document", href: "/artist-dashboard/import/" }}
        />

        <SupportingTaskDisclosure
          icon={ShieldCheck}
          label="Privacy and reuse"
          title="How private media moves through KLEIO"
          description="Open for the rules governing analysis, reuse, and institutional visibility."
          className="shadow-none"
        >
          <p className="text-sm leading-6 text-[#746E80]">Files remain private until you separately approve a Passport record, Portfolio use, profile use, or application attachment. Institutions cannot browse this library, extraction jobs, rejected proposals, or unconfirmed interpretations.</p>
        </SupportingTaskDisclosure>
        ...
        <section className="rounded-[22px] border border-[#E7E1F7] bg-white p-4 shadow-[0_14px_42px_rgba(82,64,130,0.05)]" aria-label="Search and filter private media">
