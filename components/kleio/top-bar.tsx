...
export function TopBar() {
  ...
  const demoMessage = isPreview
    ? locale === "es" ? "Acción de vista previa. El backend de producción todavía no está conectado." : "Preview action. Production backend export is not connected yet."
    : locale === "es" ? "Acción de demostración. Este prototipo no modifica datos reales." : "Demo action. This prototype does not change live data."
  ...
  const searchPlaceholder = isPreview
    ? locale === "es" ? "Buscar en KLEIO Workspace…" : "Search KLEIO Workspace…"
    : locale === "es" ? "Buscar en el demo institucional…" : "Search the institution demo…"
  ...
  return (
    <header className="sticky top-0 z-20 flex min-h-16 items-center gap-3 border-b border-border bg-background/85 px-5 py-3 backdrop-blur-xl xl:px-7">
      <div className="relative flex-1 max-w-3xl">
        ...
        <input type="search" placeholder={searchPlaceholder} className="h-10 w-full rounded-xl border border-border bg-card pl-10 pr-16 text-sm text-foreground shadow-sm outline-none transition-colors placeholder:text-muted-foreground focus:border-primary/40 focus:ring-4 focus:ring-primary/10" aria-label={t("institution.topBar.searchPlaceholder")} />
          ...
          {notificationsOpen && (
            <div className="absolute right-0 top-12 z-50 w-[24rem] overflow-hidden rounded-2xl border border-border bg-card shadow-[0_18px_50px_rgba(40,30,70,0.14)]">
              <div className="flex items-center justify-between border-b border-border px-4 py-3">
                <div><p className="text-sm font-semibold text-foreground">{notificationThreads.length} actions need attention</p><p className="text-xs text-muted-foreground">Each item opens the exact thread or workflow.</p></div>
