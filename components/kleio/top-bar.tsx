...
export function TopBar() {
  ...
  const demoMessage = locale === "es" ? "Acción de demostración. Este prototipo no modifica datos reales." : "Demo action. This prototype does not change live data."
  ...
  return (
    <header className="sticky top-0 z-20 flex min-h-16 items-center gap-3 border-b border-border bg-background/85 px-5 py-3 backdrop-blur-xl xl:px-7">
      <div className="relative flex-1 max-w-3xl">
        ...
        <input type="search" placeholder={locale === "es" ? "Buscar en el demo institucional…" : "Search the institution demo…"} className="h-10 w-full rounded-xl border border-border bg-card pl-10 pr-16 text-sm text-foreground shadow-sm outline-none transition-colors placeholder:text-muted-foreground focus:border-primary/40 focus:ring-4 focus:ring-primary/10" aria-label={t("institution.topBar.searchPlaceholder")} />
        <kbd className="absolute right-3 top-1/2 -translate-y-1/2 rounded-md border border-border bg-muted px-1.5 py-0.5 text-[0.7rem] font-medium text-muted-foreground">⌘K</kbd>
      </div>
      ...
      <div className="ml-auto flex shrink-0 items-center gap-2">
        ...
        <div className="relative">
          <button type="button" onClick={() => setNotificationsOpen((open) => !open)} aria-expanded={notificationsOpen} aria-label={locale === "es" ? "Ver tareas pendientes" : "View pending tasks"} className="relative grid size-10 place-items-center rounded-xl border border-border bg-card text-muted-foreground shadow-sm transition-colors hover:bg-accent/50 hover:text-foreground">
            ...
            <span className="absolute -right-1 -top-1 grid size-5 place-items-center rounded-full bg-primary text-[0.65rem] font-semibold text-primary-foreground ring-2 ring-background">{notificationThreads.length}</span>
          </button>
          ...
          {notificationsOpen && (
            <div className="absolute right-0 top-12 z-50 w-[24rem] overflow-hidden rounded-2xl border border-border bg-card shadow-[0_18px_50px_rgba(40,30,70,0.14)]">
              <div className="flex items-center justify-between border-b border-border px-4 py-3">
                <div>
                  <p className="text-sm font-semibold text-foreground">{notificationThreads.length} actions need attention</p>
                  <p className="text-xs text-muted-foreground">Each item opens the exact thread or workflow.</p>
                </div>
                <button type="button" onClick={() => setNotificationsOpen(false)} aria-label="Close notifications" className="grid size-7 place-items-center rounded-md text-muted-foreground transition-colors hover:bg-accent/50 hover:text-foreground"><X className="size-4" /></button>
              </div>

              <div className="max-h-[22rem] overflow-y-auto p-2">
                {notificationThreads.map((thread) => {
                  const linkedMessage = getDemoMessageForThread(thread.linkedMessageId)
                  const task = taskForThread(thread)
                  const TaskIcon = task.icon
                  return (
                    <Link key={thread.id} href={`/messages/?thread=${thread.id}`} onClick={() => setNotificationsOpen(false)} className="block rounded-xl px-3 py-3 transition-colors hover:bg-accent/40">
                      <div className="flex items-start gap-3">
                        ...
                        <span className="min-w-0 flex-1">
                          <span className="flex items-center justify-between gap-2">
                            ...
                            <span className="shrink-0 rounded-full bg-primary/10 px-2 py-0.5 text-[0.58rem] font-semibold uppercase tracking-wide text-primary">{linkedMessage?.status ?? thread.channel}</span>
                          </span>
                          <span className="mt-0.5 block text-xs font-semibold text-foreground/80">{task.title}</span>
                          <span className="mt-0.5 block text-xs leading-snug text-muted-foreground">{thread.preview}</span>
                          <span className="mt-2 inline-flex h-7 items-center gap-1.5 rounded-full bg-primary px-2.5 text-[0.65rem] font-semibold text-primary-foreground"><TaskIcon className="size-3" />{task.action}</span>
                        </span>
                      </div>
                    </Link>
                  )
              ...
              <div className="border-t border-border p-3">
                <Link href="/messages/" onClick={() => setNotificationsOpen(false)} className="inline-flex h-9 w-full items-center justify-center rounded-xl bg-primary text-xs font-semibold text-primary-foreground transition-colors hover:bg-primary/90">Open full message center</Link>
              </div>
            </div>
          )}
        </div>
