    ...
    <section className="rounded-2xl border border-[#E7E1F7] bg-white px-4 py-3 sm:px-5" aria-label="Application artwork actions">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs leading-5 text-[#746E80]">{hasPortfolio ? "Reuse approved Portfolio works here. Only add something new when this opportunity genuinely needs it." : "Keep this application open while you add a work in Portfolio in a new tab. Return here afterward and choose the exact work for this opportunity."}</p>
        </div>
        {!hasPortfolio && <Link href="/artist-dashboard/portfolio/" target="_blank" rel="noreferrer" className="inline-flex min-h-10 shrink-0 items-center justify-center gap-2 rounded-xl border border-[#D8D0F2] bg-white px-3 py-2 text-xs font-semibold text-[#5B4B8A] transition hover:border-[#B9A9DE] hover:bg-[#FBFAFE] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#A997E8]/20"><Images className="size-4" />Open Portfolio in new tab</Link>}
