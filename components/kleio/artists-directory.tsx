...
export function ArtistsDirectory() {
  ...
  const filteredArtists = useMemo(() => {
    ...
    return kleioSyntheticArtistProfiles.filter((artist) => {
      const haystack = [
        artist.displayName,
        artist.role,
        artist.location,
      ...
      <div className="mx-auto max-w-[1180px] space-y-5">
        ...
        <section className="rounded-2xl border bg-white p-5" style={{ borderColor: lavenderSoftLine, boxShadow: cardShadow }}>
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search artists, disciplines, locations, materials..."
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5">
                      <h2 className="truncate font-serif text-base font-semibold" style={{ color: inkColor }}>
                        {artist.displayName}
                      </h2>
                      <BadgeCheck className="size-4 shrink-0" style={{ color: lavenderDeep }} aria-label="Creative Passport" />
                    </div>
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {artist.practiceTags.slice(0, 4).map((tag) => (
                    <ProfileChip key={tag} label={tag} />
                  ))}
                </div>

                <div className="mt-4 space-y-2 rounded-xl border px-3 py-2.5" style={{ borderColor: lavenderSoftLine }}>
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-xs font-medium" style={{ color: inkColor }}>
                      Materials readiness
                    </span>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <Link
                    ...
                    className="inline-flex h-9 flex-1 items-center justify-center gap-1.5 rounded-xl bg-primary px-3 text-xs font-semibold text-primary-foreground transition-colors hover:bg-primary/90"
                  >
                  ...
                  <Link
                    ...
                    className="inline-flex h-9 flex-1 items-center justify-center gap-1.5 rounded-xl border px-3 text-xs font-semibold transition-colors hover:bg-[#F7F4FF]"
                    style={{ borderColor: lavenderSoftLine, color: lavenderDeep }}
                  >
        ...
        {filteredArtists.length === 0 && (
          <p className="py-8 text-center text-sm" style={{ color: mutedColor }}>
            No artists match your search. Try a different discipline, location, or name.
          </p>
        )}
      </div>
    </main>
