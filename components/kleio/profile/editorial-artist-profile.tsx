...
type EditorialArtistProfileLocale = "en" | "es"

const COPY = {
  en: {
    eyebrow: "KLEIO / Artist Profile",
    navAria: "Artist profile sections",
    worksNav: "Works",
    profileNav: "Profile",
    contactNav: "Contact",
    artistName: "Artist name",
    passport: "Creative Passport",
    intro: "Artist-selected work and approved Creative Passport information, organized in one consistent review format.",
    imagePlaceholder: "Artwork image",
    imageMissing: "Not yet added",
    untitled: "Untitled",
    portrait: "portrait",
    featuredPractice: "Featured practice",
    featuredArtworkAlt: "featured artwork",
    featuredWork: "Featured work",
    selectedWorks: "Selected works",
    portfolioSelection: "Portfolio selection",
    work: "work",
    works: "works",
    noWorksTitle: "No portfolio works added yet",
    noWorksBody: "Uploaded works will appear here in the shared KLEIO layout.",
    profileSnapshot: "Profile snapshot",
    biographyMissing: "Biography not yet added.",
    selectedRecord: "Selected record",
    artistStatement: "Artist statement",
    professionalRecord: "Professional record",
    education: "Education",
    exhibitions: "Exhibitions and residencies",
    awards: "Awards",
    contact: "Contact",
    website: "Website",
    profileControls: "Profile controls",
    structured: "Structured through the artist’s Creative Passport",
    footer: "One KLEIO layout · Artist-controlled images and information",
    backToTop: "Back to top",
  },
  es: {
    eyebrow: "KLEIO / Perfil artístico",
    navAria: "Secciones del perfil artístico",
    worksNav: "Obras",
    profileNav: "Perfil",
    contactNav: "Contacto",
    artistName: "Nombre artístico",
    passport: "Pasaporte Creativo",
    intro: "Obras seleccionadas por el artista e información aprobada del Pasaporte Creativo, organizadas en un formato de revisión coherente.",
    imagePlaceholder: "Imagen de la obra",
    imageMissing: "Aún no añadida",
    untitled: "Sin título",
    portrait: "retrato",
    featuredPractice: "Práctica destacada",
    featuredArtworkAlt: "obra destacada",
    featuredWork: "Obra destacada",
    selectedWorks: "Obras seleccionadas",
    portfolioSelection: "Selección de portafolio",
    work: "obra",
    works: "obras",
    noWorksTitle: "Aún no hay obras en el portafolio",
    noWorksBody: "Las obras que añadas aparecerán aquí dentro del formato compartido de KLEIO.",
    profileSnapshot: "Resumen del perfil",
    biographyMissing: "Biografía aún no añadida.",
    selectedRecord: "Trayectoria seleccionada",
    artistStatement: "Declaración artística",
    professionalRecord: "Trayectoria profesional",
    education: "Formación",
    exhibitions: "Exposiciones y residencias",
    awards: "Premios",
    contact: "Contacto",
    website: "Sitio web",
    profileControls: "Controles del perfil",
    structured: "Estructurado mediante el Pasaporte Creativo del artista",
    footer: "Un formato KLEIO · Imágenes e información controladas por el artista",
    backToTop: "Volver arriba",
  },
} as const

type ProfileCopy = (typeof COPY)[EditorialArtistProfileLocale]
      ...
      {src ? (
        ...
        <div className="relative grid size-full min-h-[240px] place-items-center px-8 text-center">
          ...
          <div className="relative z-10">
            <p className="font-serif text-2xl tracking-[-0.03em] text-[#4E426F]">{copy.imagePlaceholder}</p>
            <p className="mt-2 text-xs uppercase tracking-[0.16em] text-[#8A8296]">{copy.imageMissing}</p>
          </div>
        </div>
      )}
    </div>
...
function WorkCaption({ work, index, copy }: { work: EditorialArtistProfileWork; index: number; copy: ProfileCopy }) {
  ...
  return (
    <figcaption className="mt-3 flex items-start justify-between gap-4 border-t border-[#DDD7E7] pt-3 text-xs text-[#746F7C]">
      <div className="min-w-0">
        <p className="font-medium text-[#242129]">{work.title || copy.untitled}</p>
...
function PortraitOverlay({ name, src, copy }: { name: string; src?: string | null; copy: ProfileCopy }) {
  return (
    <div className="absolute bottom-4 right-4 z-20 w-24 overflow-hidden border-[5px] border-[#FCFBFD] bg-[#EEE9F4] shadow-[0_18px_50px_rgba(40,32,52,0.2)] sm:w-32 lg:-right-16 lg:bottom-8 lg:w-36">
      {src ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={src} alt={`${copy.portrait} ${name}`} className="aspect-[4/5] w-full object-cover object-center" />
      ) : (
...
function ProfileLinks({ data, copy }: { data: EditorialArtistProfileData; copy: ProfileCopy }) {
  ...
  return (
    <div id="contact" className="scroll-mt-8 border-t border-[#DDD7E7] pt-4">
      <p className="text-[0.63rem] font-semibold uppercase tracking-[0.18em] text-[#6A5896]">{copy.contact}</p>
      <div className="mt-3 grid gap-2 text-sm">
        {website && (
          <a href={website} target="_blank" rel="noreferrer" className="flex items-center justify-between gap-3 py-1.5 text-[#4F4957] hover:text-[#6A5896]">
            <span className="inline-flex items-center gap-2"><Globe className="size-3.5" />{copy.website}</span>
...
export function EditorialArtistProfile({
  ...
  actions?: ReactNode
  eyebrow?: string
  locale?: EditorialArtistProfileLocale
}) {
  ...
  const heroLabel = data.heroLabel || primaryWork?.title || copy.featuredPractice
  const recordExists = Boolean(data.education || history.length || awards.length)

  return (
    <article className="relative mx-auto w-full max-w-[1440px] overflow-hidden bg-[#FCFBFD] text-[#242129]" lang={locale}>
      ...
      <div className="relative z-10 px-4 pb-16 pt-5 sm:px-6 lg:px-10">
        <header id="top" className="border-b border-[#DDD7E7] pb-5">
          <div className="flex flex-wrap items-center justify-between gap-3 text-[0.65rem] font-semibold uppercase tracking-[0.18em] text-[#746F7C]">
            <span>{eyebrow || copy.eyebrow}</span>
            <nav className="flex items-center gap-5" aria-label={copy.navAria}>
              <a href="#works" className="hover:text-[#242129]">{copy.worksNav}</a>
              <a href="#profile" className="hover:text-[#242129]">{copy.profileNav}</a>
              <a href="#contact" className="hover:text-[#242129]">{copy.contactNav}</a>
            </nav>
          </div>

          <div className="mt-6 grid gap-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            <div>
              <h1 className="max-w-[1080px] font-serif text-[clamp(3.25rem,8vw,7.35rem)] font-medium leading-[0.84] tracking-[-0.06em]">{data.name || copy.artistName}</h1>
              <div className="mt-5 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs font-medium uppercase tracking-[0.12em] text-[#746F7C]">
                ...
                <span className="inline-flex items-center gap-1.5 text-[#6A5896]"><BadgeCheck className="size-3.5" />{data.passportLabel || copy.passport}</span>
              </div>
            </div>
            <p className="max-w-sm text-sm leading-6 text-[#746F7C] lg:text-right">{copy.intro}</p>
          </div>
        </header>
        ...
        <section className="grid gap-8 pt-6 lg:grid-cols-[minmax(0,1.55fr)_minmax(320px,0.62fr)] lg:gap-10">
          <div className="min-w-0">
            <div className="relative">
              <ArtworkFrame src={heroImage} alt={`${data.name} ${copy.featuredArtworkAlt}`} prominence="hero" copy={copy} />
              ...
              <div className="absolute bottom-4 left-4 z-20 max-w-[62%] text-white sm:bottom-5 sm:left-5">
                <p className="text-[0.62rem] font-semibold uppercase tracking-[0.18em] text-white/75">{copy.featuredWork}</p>
                <p className="mt-1 font-serif text-2xl tracking-[-0.03em] sm:text-3xl">{heroLabel}</p>
              </div>
              <PortraitOverlay name={data.name} src={data.portraitImage} copy={copy} />
            </div>
            ...
            <section id="works" className="scroll-mt-8 pt-10">
              <div className="mb-5 flex items-end justify-between gap-5 border-b border-[#DDD7E7] pb-3">
                <div>
                  <p className="text-[0.65rem] font-semibold uppercase tracking-[0.2em] text-[#6A5896]">{copy.selectedWorks}</p>
                  <h2 className="mt-2 font-serif text-3xl tracking-[-0.04em] sm:text-4xl">{copy.portfolioSelection}</h2>
                </div>
                <span className="hidden text-xs uppercase tracking-[0.14em] text-[#746F7C] sm:block">{works.length} {works.length === 1 ? copy.work : copy.works}</span>
              </div>
              ...
              {works.length ? (
                <div className="grid gap-6 md:grid-cols-12 md:items-start">
                  {primaryWork && (
                    <figure className="md:col-span-7">
                      <ArtworkFrame src={primaryWork.image} alt={primaryWork.title} prominence="primary" copy={copy} />
                      <WorkCaption work={primaryWork} index={0} copy={copy} />
                  ...
                  <div className="grid gap-6 md:col-span-5">
                    {secondaryWorks.map((work, index) => (
                      <figure key={work.id || `${work.title}-${index}`}>
                        <ArtworkFrame src={work.image} alt={work.title} copy={copy} />
                        <WorkCaption work={work} index={index + 1} copy={copy} />
                      </figure>
                    ))}
                  </div>
                </div>
                ...
                <div className="relative overflow-hidden border border-[#E2DCEE] bg-white px-6 py-14 text-center">
                  <LavenderGrain />
                  <div className="relative z-10"><p className="font-serif text-2xl text-[#4E426F]">{copy.noWorksTitle}</p><p className="mt-2 text-sm text-[#746F7C]">{copy.noWorksBody}</p></div>
                </div>
              )}
            </section>
          </div>
          ...
          <aside id="profile" className="kleio-context-panel scroll-mt-8 lg:self-start">
            <div className="relative overflow-hidden border-y border-[#DDD7E7] bg-white/84 px-1 py-5 backdrop-blur-sm sm:px-5 lg:border lg:p-5">
              <LavenderGrain />
              <div className="relative z-10">
                <p className="text-[0.63rem] font-semibold uppercase tracking-[0.18em] text-[#6A5896]">{copy.profileSnapshot}</p>
                ...
                <p className="mt-5 whitespace-pre-wrap text-sm leading-6 text-[#4F4957]">{data.bio || copy.biographyMissing}</p>
                ...
                {history.length > 0 && (
                  <div className="mt-6 border-t border-[#DDD7E7] pt-4">
                    <p className="text-[0.63rem] font-semibold uppercase tracking-[0.18em] text-[#6A5896]">{copy.selectedRecord}</p>
                ...
                {(data.artistStatement || data.practiceDescription) && (
                  <details className="group mt-4 border-t border-[#DDD7E7] pt-4">
                    <summary className="flex cursor-pointer list-none items-center justify-between gap-3 text-xs font-semibold uppercase tracking-[0.14em] text-[#4E426F]">{copy.artistStatement}<span className="text-lg font-light transition-transform group-open:rotate-45">+</span></summary>
                ...
                {recordExists && (
                  <details className="group mt-4 border-t border-[#DDD7E7] pt-4">
                    <summary className="flex cursor-pointer list-none items-center justify-between gap-3 text-xs font-semibold uppercase tracking-[0.14em] text-[#4E426F]">{copy.professionalRecord}<span className="text-lg font-light transition-transform group-open:rotate-45">+</span></summary>
                    <div className="mt-3 space-y-4 text-sm leading-6 text-[#625C70]">
                      {data.education && <div><p className="text-[0.62rem] font-semibold uppercase tracking-[0.14em] text-[#8A8296]">{copy.education}</p><p className="mt-1 whitespace-pre-wrap">{data.education}</p></div>}
                      {history.length > 3 && <div><p className="text-[0.62rem] font-semibold uppercase tracking-[0.14em] text-[#8A8296]">{copy.exhibitions}</p><ul className="mt-1 space-y-1.5">{history.map((entry, index) => <li key={`${entry}-full-${index}`}>{entry}</li>)}</ul></div>}
                      {awards.length > 0 && <div><p className="text-[0.62rem] font-semibold uppercase tracking-[0.14em] text-[#8A8296]">{copy.awards}</p><ul className="mt-1 space-y-1.5">{awards.map((entry, index) => <li key={`${entry}-award-${index}`}>{entry}</li>)}</ul></div>}
                    </div>
                  </details>
                )}

                <ProfileLinks data={data} copy={copy} />
                {actions && <div className="mt-5 border-t border-[#DDD7E7] pt-4"><p className="mb-3 text-[0.63rem] font-semibold uppercase tracking-[0.18em] text-[#6A5896]">{copy.profileControls}</p><div className="flex flex-wrap gap-2">{actions}</div></div>}
                <div className="mt-5 flex items-center gap-2 border-t border-[#DDD7E7] pt-4 text-xs text-[#746F7C]"><FileText className="size-3.5 text-[#6A5896]" />{copy.structured}</div>
              </div>
            </div>
          </aside>
        </section>
        ...
        <footer className="mt-12 flex flex-wrap items-center justify-between gap-4 border-t border-[#DDD7E7] pt-4 text-[0.65rem] uppercase tracking-[0.14em] text-[#746F7C]">
          ...
          <a href="#top" className="hover:text-[#242129]">{copy.backToTop}</a>
        </footer>
      </div>
    </article>
  )
