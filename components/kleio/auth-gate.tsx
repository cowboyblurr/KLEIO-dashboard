...
function RoleAccessButtons({ onSelect, mode }: { onSelect: (role: DemoRole, mode: KleioMode) => void; mode: KleioMode }) {
  return (
    <div className="grid gap-2">
      <button type="button" onClick={() => onSelect("institution", mode)} className="inline-flex h-9 items-center justify-center rounded-xl border border-primary/25 bg-primary/10 px-3 text-xs font-semibold text-primary transition-colors hover:bg-primary/15">Institution</button>
      <button type="button" onClick={() => onSelect("artist", mode)} className="inline-flex h-9 items-center justify-center rounded-xl border border-border bg-background px-3 text-xs font-semibold text-foreground transition-colors hover:bg-accent/50">Artist</button>
      <button type="button" onClick={() => onSelect("collaborator", mode)} className="inline-flex h-9 items-center justify-center rounded-xl border border-border bg-background px-3 text-xs font-semibold text-foreground transition-colors hover:bg-accent/50">Reviewer</button>
    </div>
  )
}
...
function demoGuideState(role: DemoRole) {
...
function AuthWall({ requiredRole, session, onRefresh }: { requiredRole?: DemoRole; session: KleioDemoSession | null; onRefresh: () => void }) {
  ...
  function handlePreviewLogin() {
    ...
    if (!nextSession) {
      setError("Those preview credentials did not match. Try institution@kleio.demo with password kleio2026.")
      return
    }
    onRefresh()
        ...
        {wrongRole ? (
          ...
          <>
            ...
            <section className="mt-6 rounded-2xl border border-border bg-background p-4">
              <p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Product preview access</p>
              <p className="mt-1 text-xs leading-relaxed text-muted-foreground">Log in to the cleaner KLEIO workspace preview.</p>
              <div className="mt-3 grid gap-2 sm:grid-cols-[1fr_0.7fr]">
                <input value={email} onChange={(event) => setEmail(event.target.value)} onKeyDown={(event) => event.key === "Enter" && handlePreviewLogin()} className="h-10 rounded-xl border border-border bg-card px-3 text-sm outline-none focus:border-primary/40 focus:ring-4 focus:ring-primary/10" placeholder="institution@kleio.demo" />
                <input value={password} onChange={(event) => setPassword(event.target.value)} onKeyDown={(event) => event.key === "Enter" && handlePreviewLogin()} type="password" className="h-10 rounded-xl border border-border bg-card px-3 text-sm outline-none focus:border-primary/40 focus:ring-4 focus:ring-primary/10" placeholder="Password" />
              </div>
              {error && <p className="mt-2 text-xs font-medium text-[oklch(0.45_0.14_55)]">{error}</p>}
              <button type="button" onClick={handlePreviewLogin} className="mt-3 inline-flex h-10 w-full items-center justify-center rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground transition-colors hover:bg-primary/90">Log in to Product Preview</button>
            </section>

            <details className="group mt-3 rounded-2xl border border-[#E7E1F7] bg-white p-4">
              <summary className="flex cursor-pointer list-none items-center justify-between text-sm font-semibold text-[#5B4B8A] marker:hidden"><span>Advanced role shortcuts</span><span className="transition-transform group-open:rotate-90">›</span></summary>
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <section className="rounded-xl border border-[#E7E1F7] bg-[#F7F4FF] p-3"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#A997E8]">Demo walkthrough</p><p className="mt-1 text-xs text-muted-foreground">Guided sample records.</p><div className="mt-3"><RoleAccessButtons onSelect={enter} mode="demo" /></div></section>
                <section className="rounded-xl border border-border bg-background p-3"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Product preview</p><p className="mt-1 text-xs text-muted-foreground">Cleaner workspace mode.</p><div className="mt-3"><RoleAccessButtons onSelect={enter} mode="preview" /></div></section>
              </div>
            </details>
