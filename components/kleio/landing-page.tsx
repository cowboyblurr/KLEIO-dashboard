    ...
    <main className="relative min-h-dvh w-full overflow-x-hidden bg-white text-[#292631]">
      <header className="relative z-30 h-[96px] w-full">
        <div className="relative mx-auto h-full w-full max-w-[1280px] px-8 max-md:px-5">
          ...
          <nav className="absolute right-8 top-1/2 flex -translate-y-1/2 items-center gap-7 max-md:right-5 max-md:gap-4"><Link href="/demo/" className="text-[0.78rem] font-medium tracking-wide hover:opacity-70 max-md:hidden" style={navLinkStyle}>{isSpanish ? "Recorrido" : "Product Tour"}</Link><KleioLocaleToggle /></nav>
        </div>
      </header>
        ...
        <div className="landing-card-grid mx-auto grid w-full max-w-[1040px] grid-cols-[minmax(320px,0.88fr)_24px_minmax(320px,0.78fr)] items-start gap-4 max-lg:grid-cols-1 max-lg:gap-4">
          ...
          <div className="grid gap-3">
            <div className="landing-choice-card flex flex-col rounded-[1.1rem] p-3.5" style={{ backgroundColor: cardBg, border: `1px solid ${lavenderSoftLine}`, boxShadow: cardShadow }}>
              <p className="text-[0.62rem] font-semibold uppercase tracking-[0.16em]" style={{ color: lavenderAccent }}>{isSpanish ? "Vista previa privada" : "Private pilot preview"}</p>
              ...
              <p className="mt-0.5 text-[0.68rem] leading-relaxed" style={{ color: mutedColor }}>{isSpanish ? "Comienza desde el lado artista o institucional para ver cómo KLEIO organiza materiales, postulaciones y revisión." : "Start from the artist or institution side to see how KLEIO organizes materials, submissions, and review."}</p>
            ...
            <div className="rounded-[1.1rem] border border-[#E7E1F7] bg-[#F7F4FF] p-3.5 shadow-[0_18px_48px_rgba(82,64,130,0.08)]">
              <p className="text-[0.62rem] font-semibold uppercase tracking-[0.16em] text-[#A997E8]">{isSpanish ? "Recorrido guiado" : "Guided walkthrough"}</p>
              <h2 className="mt-1 font-serif text-[0.98rem] font-semibold text-[#292631]">{isSpanish ? "Explora el flujo institucional" : "Explore the institution flow"}</h2>
              <p className="mt-1 text-[0.68rem] leading-relaxed text-[#7F7890]">{isSpanish ? "Sigue un recorrido de producto para ver cómo se conectan postulaciones, revisores, comité, shortlist e informes." : "Follow a product walkthrough to see how submissions, reviewers, committee review, shortlist movement, and reports connect."}</p>
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                <button type="button" onClick={startInstitutionDemo} className="inline-flex h-9 items-center justify-center gap-1 rounded-full bg-[#5B4B8A] px-4 text-[0.7rem] font-semibold text-white shadow-[0_10px_24px_rgba(82,64,130,0.16)] transition-opacity hover:opacity-90">
                  {isSpanish ? "Empezar recorrido" : "Start guided flow"}
                ...
                <button type="button" onClick={startGuidedDemo} className="inline-flex h-9 items-center justify-center gap-1 rounded-full border border-[#D8D0F2] bg-white px-4 text-[0.7rem] font-semibold text-[#5B4B8A] transition-colors hover:bg-white/75">
                  {isSpanish ? "Abrir tour completo" : "Open full tour"}
                  <ChevronRight className="size-3" />
                </button>
              </div>
            </div>
