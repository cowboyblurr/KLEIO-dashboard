        ...
        <section className="rounded-[28px] border border-[#DED7EF] bg-white p-5 shadow-[0_18px_52px_rgba(82,64,130,0.06)] sm:p-6" aria-labelledby="private-material-upload-title">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="max-w-3xl">
              <p className="text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-[#75639E]">{locale === "es" ? "Carga privada" : "Private upload"}</p>
              <h1 id="private-material-upload-title" className="mt-1 font-serif text-2xl font-semibold tracking-[-0.03em] text-[#292631]">{locale === "es" ? "Añade medios o archivos de apoyo" : "Add media or supporting files"}</h1>
              <p className="mt-2 text-sm leading-6 text-[#746E80]">{locale === "es" ? "Sube imágenes, video, audio y documentos comunes a tu Biblioteca privada de KLEIO. Después decide dónde reutilizarlos. Los PDF también pueden pasar al análisis documental de Gemini más abajo." : "Upload images, video, audio, and common documents to your private KLEIO Library, then decide where to reuse them. PDFs can also enter Gemini document analysis farther down this page."}</p>
            </div>
            <QuickMediaImport context="existing_media_library" label={locale === "es" ? "Subir medios" : "Upload media"} onConfirm={() => undefined} />
          </div>
          <p className="mt-4 text-xs font-semibold text-[#6A5896]"><ShieldCheck className="mr-1.5 inline size-3.5" />{locale === "es" ? "Privado por defecto · nada se publica ni se adjunta hasta que lo confirmes" : "Private by default · nothing is published or attached until you confirm a destination"}</p>
        </section>
