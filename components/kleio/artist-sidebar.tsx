...
/* eslint-disable @next/next/no-img-element -- artist profile images use short-lived signed Supabase URLs */
...
import { loadEnhancedArtistProfile } from "@/lib/kleio-artist-profile"
...
export function ArtistSidebar() {
  ...
  useEffect(() => {
    ...
    void Promise.all([loadKleioAccount(), loadEnhancedArtistProfile()])
      .then(([account, profile]) => {
        if (!active || !account) return
        setLiveArtist({
          name: profile?.professional_name || account.profile.display_name || account.user.email?.split("@")[0] || "KLEIO Artist",
          discipline: profile?.disciplines[0] ? disciplineLabel(profile.disciplines[0], locale) : (locale === "es" ? "Artista" : "Artist"),
          imageUrl: profile?.profile_image_url || null,
          positionX: profile?.profile_image_position_x ?? 50,
          positionY: profile?.profile_image_position_y ?? 50,
        })
      })
        ...
        <nav className="flex-1 overflow-y-auto px-3 pb-4">
          {navSections.map((section) => (
            <div key={section.heading} className="mb-5">
              <p className="px-3 pb-2 text-[0.65rem] font-semibold uppercase tracking-[0.16em] text-muted-foreground/80">{locale === "es" ? section.headingEs : section.heading}</p>
