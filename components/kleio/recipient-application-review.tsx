...
const sectionIds = ["overview", "responses", "works", "documents", "artist", "communication"] as const
...
export function RecipientApplicationReview() {
  ...
  const responses = useMemo(() => {
    if (!snapshot) return []
    return [
      snapshot.introduction ? { label: "Application introduction", answer: snapshot.introduction } : null,
      snapshot.opportunity_response ? { label: "Opportunity-specific response", answer: snapshot.opportunity_response } : null,
    ].filter((item): item is { label: string; answer: string } => Boolean(item))
  }, [snapshot])
  const proposalSynopsis = synopsisFrom(snapshot?.opportunity_response || snapshot?.introduction || "")
  ...
  return (
    <main className="min-h-dvh bg-[#F8F5EF] text-[#2D2931] selection:bg-[#E6DDEF] selection:text-[#2D2931]">
        ...
        <div className="mx-auto max-w-[1320px] px-4 py-10 sm:px-6 sm:py-14 lg:px-8 lg:py-16">
          <div className="max-w-5xl">
            <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
              ...
              {isSynthetic && <span className="rounded-full border border-amber-300 bg-amber-50 px-2.5 py-1 text-[0.68rem] font-semibold text-amber-800">Internal synthetic test</span>}
            </div>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-x-2 gap-y-1 text-sm leading-6 text-[#6F6873]">
              ...
              {snapshot.artist.location && <><span aria-hidden="true">·</span><span>{snapshot.artist.location}</span></>}
              {portfolioMedia.length > 0 && <><span aria-hidden="true">·</span><span>{portfolioMedia.join(" · ")}</span></>}
            </div>
            <div className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-xs text-[#88808D]">
              ...
              {approvedDate && <><span aria-hidden="true">·</span><span>Approved {approvedDate}</span></>}
            </div>
          ...
          {snapshot.synthetic_notice && <div role="note" className="mt-8 max-w-4xl border-l-2 border-amber-400 pl-4 text-sm leading-6 text-amber-900">{snapshot.synthetic_notice}</div>}
        </div>
      </section>
        ...
        <div className="mx-auto max-w-[1320px] overflow-x-auto px-4 sm:px-6 lg:px-8">
          <div className="flex min-w-max items-center gap-1 py-2">
            {[
              ...
              <button key={id} type="button" onClick={() => scrollToSection(id as SectionId)} aria-current={activeSection === id ? "location" : undefined} className={`rounded-full px-3.5 py-2 text-xs font-semibold transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#8B79B6]/20 ${activeSection === id ? "bg-[#E9E1EE] text-[#4A3B59]" : "text-[#766F7A] hover:bg-[#F1EDF0] hover:text-[#403653]"}`}>{label}</button>
            ))}
          </div>
        </div>
      </nav>
      ...
      <div className="mx-auto max-w-[1320px] px-4 pb-16 pt-5 sm:px-6 lg:px-8 lg:pb-24">
        {status && <div role="status" aria-live="polite" className="mb-5 border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm leading-6 text-emerald-800 print:hidden">{status}</div>}
        {error && <div role="alert" className="mb-5 border border-red-200 bg-red-50 px-4 py-3 text-sm leading-6 text-red-800 print:hidden">{error}</div>}
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_270px] lg:gap-14 xl:gap-20">
          <div className="min-w-0">
            <section id="overview" className="scroll-mt-32 border-b border-[#DDD7CF] py-12 sm:py-16" aria-labelledby="overview-title">
              ...
              <dl className="mt-9 grid border-y border-[#DDD7CF] sm:grid-cols-2 xl:grid-cols-4">
                <div className="border-b border-[#DDD7CF] py-5 sm:border-r sm:px-5 sm:first:pl-0 xl:border-b-0"><dt className="text-[0.68rem] font-semibold uppercase tracking-[0.14em] text-[#88808D]">Applicant</dt><dd className="mt-2 font-serif text-xl font-semibold">{snapshot.artist.professional_name}</dd></div>
                <div className="border-b border-[#DDD7CF] py-5 sm:px-5 xl:border-b-0 xl:border-r"><dt className="text-[0.68rem] font-semibold uppercase tracking-[0.14em] text-[#88808D]">Location</dt><dd className="mt-2 text-sm font-medium leading-6">{snapshot.artist.location || "Not specified in this submission"}</dd></div>
                <div className="border-b border-[#DDD7CF] py-5 sm:border-r sm:px-5 sm:border-b-0"><dt className="text-[0.68rem] font-semibold uppercase tracking-[0.14em] text-[#88808D]">Materials</dt><dd className="mt-2 text-sm font-medium leading-6">{materialCount} item{materialCount === 1 ? "" : "s"} visible in this dossier</dd></div>
                <div className="py-5 sm:px-5 sm:pr-0"><dt className="text-[0.68rem] font-semibold uppercase tracking-[0.14em] text-[#88808D]">Secure access</dt><dd className="mt-2 text-sm font-medium leading-6">Expires {formatDateOnly(review.access.expires_at)}</dd></div>
              </dl>
            ...
            <section id="responses" className="scroll-mt-32 border-b border-[#DDD7CF] py-12 sm:py-16" aria-labelledby="responses-title">
              ...
              <div id="responses-title" className="mt-10">
                {responses.length > 0 ? responses.map((response, index) => (
                  <article key={response.label} className="grid gap-4 border-t border-[#DDD7CF] py-8 sm:grid-cols-[64px_minmax(0,1fr)] sm:gap-7 sm:py-10">
                ...
                )) : <p className="border-t border-[#DDD7CF] py-8 text-sm leading-7 text-[#766F7A]">No separate written responses were included in the approved snapshot.</p>}
              </div>
                ...
                {snapshot.portfolio.length > 0 ? snapshot.portfolio.map((work, index) => (
                  <article key={work.id || `${work.title}-${index}`} className={workView === "grid" ? "min-w-0" : "border-t border-[#DDD7CF] pt-8 sm:pt-10"}>
                    <button type="button" className={`group relative block w-full overflow-hidden bg-[#EEEAE4] text-left focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#8B79B6]/25 ${workView === "grid" ? "aspect-[4/3]" : "min-h-[360px] sm:min-h-[520px] lg:min-h-[620px]"}`} onClick={() => void openArtwork(work)} aria-label={`Open ${work.title || "selected artwork"} in focus view`}>
                      {work.image_url ? <img src={work.image_url} alt={work.title ? `${work.title} by ${snapshot.artist.professional_name}` : `Selected artwork by ${snapshot.artist.professional_name}`} className="size-full object-contain transition duration-500 group-hover:scale-[1.01]" /> : <span className="grid size-full min-h-[280px] place-items-center px-6 text-center text-sm leading-6 text-[#837B86]">This artwork is temporarily unavailable. The remaining submission materials are still accessible.</span>}
                      {work.image_url && <span className="absolute bottom-3 right-3 inline-flex size-9 items-center justify-center rounded-full bg-[#FFFDFC]/92 text-[#403653] opacity-0 shadow-sm transition group-hover:opacity-100 group-focus-visible:opacity-100 print:hidden"><Maximize2 className="size-4" /></span>}
                    </button>
                    <div className={workView === "grid" ? "pt-4" : "grid gap-4 pt-5 sm:grid-cols-[minmax(0,1fr)_minmax(220px,0.55fr)] sm:gap-10 sm:pt-7"}>
                      ...
                      {work.description && <p className="text-sm leading-7 text-[#5F5863]">{work.description}</p>}
                    </div>
                  </article>
            ...
            <section id="documents" className="scroll-mt-32 border-b border-[#DDD7CF] py-12 sm:py-16" aria-labelledby="documents-title">
              ...
              <div id="documents-title" className="mt-10 border-t border-[#DDD7CF]">
                ...
                {!snapshot.documents.cv_url && attachmentLabels.length === 0 && <div className="border-b border-[#DDD7CF] py-7 text-sm leading-7 text-[#766F7A]">No separate supporting documents were approved for this review page.</div>}
              </div>
              {attachmentLabels.length > 0 && <p className="mt-4 max-w-3xl text-xs leading-5 text-[#8B838F]">Where a browser preview is unavailable, KLEIO preserves the original material in the submission package rather than altering its content.</p>}
            </section>
            ...
            <section id="artist" className="scroll-mt-32 border-b border-[#DDD7CF] py-12 sm:py-16" aria-labelledby="artist-title">
              ...
              <div id="artist-title" className="mt-10 grid gap-10 xl:grid-cols-[minmax(0,1fr)_260px]">
                <div className="space-y-9">
                  <div><h3 className="font-serif text-2xl font-semibold">{snapshot.artist.professional_name}</h3>{snapshot.artist.location && <p className="mt-2 text-sm text-[#766F7A]">{snapshot.artist.location}</p>}</div>
                  {snapshot.artist.bio && <div><h4 className="text-xs font-semibold uppercase tracking-[0.14em] text-[#88808D]">Biography</h4><p className="mt-3 whitespace-pre-wrap text-[0.98rem] leading-8 text-[#4F4953]">{snapshot.artist.bio}</p></div>}
                  {snapshot.artist.artist_statement && <div><h4 className="text-xs font-semibold uppercase tracking-[0.14em] text-[#88808D]">Artist statement</h4><p className="mt-3 whitespace-pre-wrap text-[0.98rem] leading-8 text-[#4F4953]">{snapshot.artist.artist_statement}</p></div>}
                </div>
                <aside className="border-l border-[#DDD7CF] pl-5 sm:pl-6">
                  ...
                  {snapshot.artist.website_url && <a className={`${quietAction} mt-4 -ml-3`} href={snapshot.artist.website_url} target="_blank" rel="noreferrer"><ExternalLink className="size-4" />Approved artist website</a>}
                  {hasConversation && <button type="button" className={`${secondary} mt-5 w-full`} disabled={profileRequested} onClick={() => void requestMoreProfile()}>{profileRequested ? "Request sent" : "Request full Creative Passport"}</button>}
                </aside>
              </div>
            </section>
            ...
            <div className="sticky top-[118px] space-y-8 py-12">
              <div className="border-t border-[#CFC7CF] pt-5">
                ...
                <dl className="mt-5 space-y-5 text-sm">
                  ...
                  {snapshot.opportunity.provider_name && <div><dt className="text-xs text-[#8B838F]">Receiving organization</dt><dd className="mt-1 font-medium leading-6 text-[#4E4752]">{snapshot.opportunity.provider_name}</dd></div>}
      ...
      {focusedWork && (
        <div className="fixed inset-0 z-[70] grid place-items-center bg-[#201C24]/80 p-3 backdrop-blur-sm print:hidden" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setFocusedWork(null) }}>
          <div role="dialog" aria-modal="true" aria-label={`${focusedWork.title || "Artwork"} focus view`} className="flex max-h-[94dvh] w-full max-w-[1180px] flex-col overflow-hidden bg-[#F8F5EF] shadow-2xl">
            <header className="flex items-center justify-between gap-4 border-b border-[#D7D0C9] px-4 py-3 sm:px-5"><div className="min-w-0"><h2 className="truncate font-serif text-lg font-semibold">{focusedWork.title || "Untitled"}</h2><p className="truncate text-xs text-[#807884]">{[focusedWork.year, focusedWork.medium, focusedWork.dimensions].filter(Boolean).join(" · ")}</p></div><button type="button" aria-label="Close artwork focus view" className="grid size-10 shrink-0 place-items-center rounded-full hover:bg-[#ECE7E2] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#8B79B6]/20" onClick={() => setFocusedWork(null)}><X className="size-4" /></button></header>
            <div className="min-h-0 flex-1 overflow-auto bg-[#ECE8E2] p-4 sm:p-7">{focusedWork.image_url ? <img src={focusedWork.image_url} alt={focusedWork.title ? `${focusedWork.title} by ${snapshot.artist.professional_name}` : `Selected artwork by ${snapshot.artist.professional_name}`} className="mx-auto max-h-[76dvh] max-w-full object-contain" /> : <div className="grid min-h-[55dvh] place-items-center text-center text-sm leading-6 text-[#766F7A]">This artwork is temporarily unavailable. The remaining submission materials are still accessible.</div>}</div>
            {focusedWork.description && <div className="border-t border-[#D7D0C9] bg-[#FCFAF6] px-5 py-4 text-sm leading-6 text-[#5E5762]">{focusedWork.description}</div>}
          </div>
        </div>
      )}
    </main>
