...
import { loadImportAvailability, type KleioImportAvailability } from "@/lib/kleio-import-source-availability"

const card = "flex min-h-48 flex-col rounded-[24px] border border-[#D8D0F2] bg-white p-5 text-left shadow-[0_18px_48px_rgba(82,64,130,0.05)]"
...
const COPY = {
  en: {
    ...
    uploadFoot: "Use the private Add media control above.",
  ...
  es: {
    ...
    uploadFoot: "Usa el control privado Añadir medios que aparece arriba.",
...
export function ImportSourceHub() {
  ...
  const [availability, setAvailability] = useState<KleioImportAvailability | null>(null)
  useEffect(() => { void loadImportAvailability().then(setAvailability).catch(() => setAvailability(null)) }, [])
  const directAvailable = availability === null || (availability.device_document && availability.device_image && availability.device_video && availability.device_audio)
  const libraryAvailable = availability === null || availability.existing_kleio_media
  ...
  return <section className="rounded-[28px] border border-[#E2DCF1] bg-[linear-gradient(145deg,#F8F5FF,#FFFFFF)] p-5 shadow-[0_22px_70px_rgba(82,64,130,0.07)] sm:p-7" aria-labelledby="import-source-title">
    ...
    <div className="mt-6 grid gap-4 md:grid-cols-2">
      {directAvailable && <div className={card}><div className="flex items-start justify-between gap-3"><span className="grid size-11 place-items-center rounded-2xl bg-[#F0EAFB] text-[#5B4B8A]"><Images className="size-5" /></span><span className="rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[0.65rem] font-semibold text-emerald-800">{copy.available}</span></div><h3 className="mt-5 font-serif text-xl font-semibold">{copy.uploadTitle}</h3><p className="mt-2 flex-1 text-sm leading-6 text-[#746E80]">{copy.uploadBody}</p><p className="mt-4 text-xs font-semibold text-[#5B4B8A]">{copy.uploadFoot}</p></div>}
      {libraryAvailable && <div className={card}><div className="flex items-start justify-between gap-3"><span className="grid size-11 place-items-center rounded-2xl bg-[#F0EAFB] text-[#5B4B8A]"><Library className="size-5" /></span><span className="rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[0.65rem] font-semibold text-emerald-800">{copy.available}</span></div><h3 className="mt-5 font-serif text-xl font-semibold">{copy.reuseTitle}</h3><p className="mt-2 flex-1 text-sm leading-6 text-[#746E80]">{copy.reuseBody}</p><p className="mt-4 text-xs font-semibold text-[#5B4B8A]">{copy.reuseFoot}</p></div>}
    </div>
  </section>
}
