...
const COPY = {
  en: {
    ...
    active: "Active for beta", available: "Available", unavailable: "Unavailable",
    ...
    reuseBody: "Existing owner-scoped images, video, audio, and documents can be selected again without creating duplicate files. PDFs that need document intelligence can still be analyzed separately.",
    reuseFoot: "Manage everything together in your Media Library.",
    deferredEyebrow: "Deferred connected sources", deferredTitle: "Preserved for a later release decision",
    deferredBody: "Connected providers remain disabled until their reliability and permission flows are ready. Direct device upload and your private KLEIO Library remain the dependable beta paths for supported media and documents.",
    deferred: "Deferred", deferredAria: "Deferred import sources", websiteImport: "Website Import",
  },
  es: {
    eyebrow: "Importación privada de materiales",
    title: "Lleva tus archivos a KLEIO desde un solo lugar",
    intro: "Sube imágenes, video, audio y documentos de apoyo comunes desde tu dispositivo. KLEIO guarda cada fuente de forma privada y espera tu aprobación antes de reutilizarla. Los PDF también pueden entrar al análisis documental de Gemini cuando elijas ese flujo.",
    private: "Nada se publica automáticamente",
    limits: "Medios + documentos de apoyo",
    active: "Activo en la beta", available: "Disponible", unavailable: "No disponible",
    ...
    reuseBody: "Puedes volver a elegir imágenes, video, audio y documentos privados sin crear copias duplicadas. Los PDF que necesiten inteligencia documental se pueden analizar por separado.",
    reuseFoot: "Gestiona todo junto en tu Biblioteca de medios.",
    deferredEyebrow: "Fuentes conectadas pospuestas", deferredTitle: "Conservadas para una fase posterior",
    deferredBody: "Los proveedores conectados permanecen desactivados hasta que sus permisos y fiabilidad estén listos. La carga directa y la Biblioteca privada de KLEIO son las rutas estables para medios y documentos compatibles.",
    deferred: "Pospuesto", deferredAria: "Fuentes de importación pospuestas", websiteImport: "Importación web",
  },
} as const
...
export function ImportSourceHub() {
  ...
  return <section className="rounded-[28px] border border-[#E2DCF1] bg-[linear-gradient(145deg,#F8F5FF,#FFFFFF)] p-5 shadow-[0_22px_70px_rgba(82,64,130,0.07)] sm:p-7" aria-labelledby="import-source-title">
    ...
    <section className="mt-6 rounded-[22px] border border-[#E7E1F7] bg-white/70 p-4 sm:p-5"><p className="text-[0.66rem] font-semibold uppercase tracking-[0.15em] text-[#81788E]">{copy.deferredEyebrow}</p><h3 className="mt-1 font-serif text-xl font-semibold text-[#625C70]">{copy.deferredTitle}</h3><p className="mt-2 text-sm leading-6 text-[#81788E]">{copy.deferredBody}</p><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4" aria-label={copy.deferredAria}><div className="rounded-xl border border-[#E7E1F7] bg-[#FAF9FD] px-4 py-3 text-sm text-[#746E80]"><Cloud className="mb-2 size-4" /><strong>Google Drive</strong><span className="block text-xs font-semibold uppercase tracking-wide">{copy.deferred}</span></div><div className="rounded-xl border border-[#E7E1F7] bg-[#FAF9FD] px-4 py-3 text-sm text-[#746E80]"><Images className="mb-2 size-4" /><strong>Instagram</strong><span className="block text-xs font-semibold uppercase tracking-wide">{copy.deferred}</span></div><div className="rounded-xl border border-[#E7E1F7] bg-[#FAF9FD] px-4 py-3 text-sm text-[#746E80]"><Cloud className="mb-2 size-4" /><strong>{copy.websiteImport}</strong><span className="block text-xs font-semibold uppercase tracking-wide">{copy.deferred}</span></div><div className="rounded-xl border border-[#E7E1F7] bg-[#FAF9FD] px-4 py-3 text-sm text-[#746E80]"><Images className="mb-2 size-4" /><strong>Pinterest</strong><span className="block text-xs font-semibold uppercase tracking-wide">{copy.deferred}</span></div></div></section>
  </section>
}
