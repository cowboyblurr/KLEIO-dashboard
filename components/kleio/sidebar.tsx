...
export function Sidebar() {
  ...
  useEffect(() => {
    ...
    Promise.all([loadKleioAccount(), loadInstitutionProfile()])
      ...
      .catch(() => {
        if (active) setLiveIdentity(null)
      })
      ...
      <div className="border-t border-border p-3">
        <Link href="/settings/" className="flex w-full items-center gap-3 rounded-lg px-2 py-2 text-left transition-colors hover:bg-accent/60" aria-label={es ? "Abrir configuración de la cuenta" : "Open account settings"}>
          ...
          <span className="min-w-0 flex-1">
            <span className="block truncate text-sm font-medium text-foreground">{personName}</span>
            <span className="block truncate text-xs text-muted-foreground">{personRole}</span>
          </span>
        ...
        <div className="mt-1 flex w-full items-center gap-3 rounded-lg border border-border bg-card px-2 py-2 text-left">
          ...
          <span className="min-w-0 flex-1">
            <span className="block truncate text-sm font-medium text-foreground">{institutionName}</span>
            <span className="block truncate text-xs text-muted-foreground">{isLive ? (es ? "Espacio institucional activo" : "Active institution workspace") : (es ? "Un solo espacio institucional" : "Single institution workspace")}</span>
          </span>
