...
import { Check, ChevronDown, ImageIcon, Images, Loader2, Pencil, Plus, Search, Sparkles, Trash2, X } from "lucide-react"
...
function WorkEditor({ draft, saving, onChange, onSave, onRemove }: {
  draft: DraftWork
  saving: boolean
...
export function VisualArtistPortfolioStudio() {
  ...
  function addDraftItems(items: ArtistMediaLibraryItem[]) {
    ...
    setEditing(null)
    setMessage(`${newDrafts.length} image${newDrafts.length === 1 ? " is" : "s are"} ready for a quick artwork review.`)
  }
  ...
  async function saveEdit() {
    ...
    try {
      ...
      await refresh(); setEditing(null); setMessage("Artwork details updated.")
  ...
  async function removeWork(work: PortfolioWorkRecord) {
    ...
    setError("")
    try { await deletePortfolioWorkPreservingLibrary(work); setWorks((current) => current.filter((item) => item.id !== work.id)); if (editing?.id === work.id) setEditing(null); setMessage("The portfolio record was removed. Its media remains private in your KLEIO Library.") }
  ...
  return (
    <main className="h-full overflow-y-auto bg-[#FCFBFE] px-4 py-5 sm:px-6 sm:py-6">
      <div className="mx-auto max-w-[1240px] space-y-5">
        <WorkspacePageHeader eyebrow="Artist workspace" title="Portfolio" description="Build your portfolio visually. Start with the work, then add only the details that make it easier to reuse in applications." />

        <section className="relative overflow-hidden rounded-[28px] border border-[#DED7EF] bg-[linear-gradient(135deg,#F5F0FE_0%,#FFFFFF_55%,#F7F4FF_100%)] p-5 shadow-[0_24px_70px_rgba(82,64,130,0.08)] sm:p-7">
          <div aria-hidden="true" className="absolute -right-16 -top-20 size-64 rounded-full bg-[#E6DDF8]/80 blur-3xl" />
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between"><div className="max-w-2xl"><p className="text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-[#75639E]">Visual-first portfolio building</p><h2 className="mt-2 font-serif text-3xl font-semibold tracking-[-0.04em]">Choose the work first. Add details second.</h2><p className="mt-3 text-sm leading-7 text-[#746E80]">Upload several images, reuse media you already added, and see everything before filling out metadata. No long blank form stands between you and the artwork.</p></div><div className="flex flex-wrap gap-2"><ArtistImportStudio compact onPortfolioChanged={() => void refresh()} /><QuickMediaImport context="portfolio" label="Choose media" onConfirm={({ items }) => addDraftItems(items)} /></div></div>
        </section>
        ...
        {drafts.length > 0 && <section className="space-y-3"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#75639E]">New work queue</p><p className="mt-1 text-sm text-[#746E80]">Review one image at a time. The rest stay safely in the tray.</p></div><span className="rounded-full bg-[#EEE8FA] px-3 py-1.5 text-xs font-semibold text-[#5B4B8A]">{activeDraft + 1} of {drafts.length}</span></div><div className="flex gap-2 overflow-x-auto pb-1" aria-label="Artwork draft tray">{drafts.map((draft, index) => <button key={draft.item.id} type="button" aria-current={activeDraft === index ? "true" : undefined} onClick={() => { setActiveDraft(index); setEditing(null) }} className={`relative min-w-36 overflow-hidden rounded-2xl border bg-white text-left focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#A997E8]/20 ${activeDraft === index ? "border-[#8C78BF] ring-2 ring-[#A997E8]/20" : "border-[#E7E1F7]"}`}>{draft.item.previewUrl ? <img src={draft.item.previewUrl} alt="" className="aspect-[4/3] w-full object-cover" /> : <div className="grid aspect-[4/3] place-items-center bg-[#F4F1F8]"><ImageIcon className="size-5 text-[#8D80AA]" /></div>}<span className="block truncate px-3 py-2 text-xs font-semibold">{draft.title || draft.item.originalFilename}</span><span className="absolute left-2 top-2 grid size-6 place-items-center rounded-full bg-white/95 text-[0.65rem] font-bold text-[#5B4B8A]">{index + 1}</span></button>)}</div>{active && <WorkEditor draft={active} saving={saving} onChange={(next) => setDrafts((current) => current.map((draft, index) => index === activeDraft ? next : draft))} onSave={() => void saveActiveDraft()} onRemove={() => { const remaining = drafts.filter((_, index) => index !== activeDraft); setDrafts(remaining); setActiveDraft(Math.max(0, Math.min(activeDraft, remaining.length - 1))) }} />}</section>}

        {editing && <section className={`${surface} overflow-hidden`}><div className="flex items-center justify-between border-b border-[#E7E1F7] px-5 py-4"><div><p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#75639E]">Edit existing work</p><h2 className="mt-1 font-serif text-2xl font-semibold">{editing.title}</h2></div><button type="button" className="grid size-10 place-items-center rounded-xl border border-[#E7E1F7]" onClick={() => setEditing(null)} aria-label="Close artwork editor"><X className="size-4" /></button></div><div className="grid lg:grid-cols-[minmax(0,1fr)_380px]"><div className="grid min-h-80 place-items-center bg-[#F2EFF7] p-6">{editing.image_url ? <img src={editing.image_url} alt={editing.title} className="max-h-[62dvh] w-full rounded-2xl object-contain shadow-[0_20px_60px_rgba(58,43,92,0.15)]" /> : <ImageIcon className="size-10 text-[#8D80AA]" />}</div><div className="grid content-start gap-4 border-t border-[#E7E1F7] p-5 lg:border-l lg:border-t-0"><label className="grid gap-1.5 text-xs font-semibold text-[#625C70]"><span>Title</span><input className={field} value={editing.title} onChange={(event) => setEditing({ ...editing, title: event.target.value })} /></label><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1"><label className="grid gap-1.5 text-xs font-semibold text-[#625C70]"><span>Year</span><input className={field} value={editing.year} onChange={(event) => setEditing({ ...editing, year: event.target.value })} /></label><label className="grid gap-1.5 text-xs font-semibold text-[#625C70]"><span>Medium</span><input className={field} value={editing.medium} onChange={(event) => setEditing({ ...editing, medium: event.target.value })} /></label></div><label className="grid gap-1.5 text-xs font-semibold text-[#625C70]"><span>Dimensions</span><input className={field} value={editing.dimensions} onChange={(event) => setEditing({ ...editing, dimensions: event.target.value })} /></label><details className="rounded-2xl border border-[#E7E1F7] p-4"><summary className="cursor-pointer text-sm font-semibold text-[#5B4B8A]">More artwork details</summary><div className="mt-4 grid gap-4"><label className="grid gap-1.5 text-xs font-semibold text-[#625C70]"><span>Description</span><textarea rows={4} className={`${field} resize-y`} value={editing.description} onChange={(event) => setEditing({ ...editing, description: event.target.value })} /></label><label className="grid gap-1.5 text-xs font-semibold text-[#625C70]"><span>Series</span><input className={field} value={editing.series} onChange={(event) => setEditing({ ...editing, series: event.target.value })} /></label><label className="grid gap-1.5 text-xs font-semibold text-[#625C70]"><span>Keywords</span><input className={field} value={editing.tags.join(", ")} onChange={(event) => setEditing({ ...editing, tags: event.target.value.split(/[,;\n]/).map((value) => value.trim()).filter(Boolean) })} /></label></div></details><div className="flex flex-wrap gap-2"><button type="button" className={primary} disabled={saving || !editing.title.trim()} onClick={() => void saveEdit()}>{saving ? <Loader2 className="size-4 animate-spin" /> : <Check className="size-4" />}Save changes</button><button type="button" className={secondary} onClick={() => void removeWork(editing)}><Trash2 className="size-4" />Remove work</button></div></div></div></section>}
