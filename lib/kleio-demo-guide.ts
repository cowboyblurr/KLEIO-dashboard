...
export const demoGuideScenarios: DemoGuideScenario[] = [
  ...
  },
  {
    ...
    completionMessage: "This walkthrough is complete. You have seen how KLEIO keeps review context visible from intake through shortlist decisions.",
...
function buildSteps(scenarioId: DemoGuideScenarioId, steps: StepSeed[]): DemoGuideStep[] {
  return steps.map((step, index) => ({ ...step, id: `${scenarioId}-${index + 1}`, scenarioId, stepNumber: index + 1, primaryActionLabel: step.primaryActionLabel ?? "Next", nextStepId: index < steps.length - 1 ? `${scenarioId}-${index + 2}` : undefined }))
}
...
const demoGuideSteps: DemoGuideStep[] = [
  ...
  ...buildSteps("review-and-shortlist", [
    { route: "/dashboard/", title: "Begin in the overview", body: "The overview gives the institution a shared read on the review cycle before anyone opens an individual application.", screenLabel: "Institution workspace · Overview", screenCue: "Look for total applications, review status, incomplete materials, reviewer progress, deadlines, and shortlist activity.", viewerAction: "This helps administrators and committee members see where attention is needed before decisions are made.", nextPreview: "Next, the guide opens Review Queue.", primaryActionLabel: "Next: Review Queue", requiredRole: "institution" },
    { route: "/review-queue/", title: "Open Review Queue", body: "The Review Queue organizes submissions that need attention. It brings readiness, priority, reviewer assignment, and status into one working view.", screenLabel: "Institution workspace · Review Queue", screenCue: "Look for the submission table, completeness indicators, assigned reviewers, priorities, and status filters.", viewerAction: "This reduces context switching. The reviewer does not have to search across email, PDFs, spreadsheets, and separate notes to understand what needs review.", nextPreview: "Next, stay in the queue and focus on the review context.", primaryActionLabel: "Next: Review context", requiredRole: "institution", targetAction: "open_submission" },
    { route: "/review-queue/", title: "Read review context", body: "A strong review flow keeps artist materials, program fit, notes, rubric context, and committee progress close to the submission.", screenLabel: "Institution workspace · Applicant context", screenCue: "Stay in Review Queue and look at how the submission can be reviewed without losing surrounding context.", viewerAction: "KLEIO supports the decision process; it does not make the decision. The committee still evaluates the work, records notes, and approves next steps.", nextPreview: "Next, the guide opens Shortlist.", primaryActionLabel: "Next: Shortlist", requiredRole: "institution" },
    { route: "/shortlist/", title: "Open Shortlist", body: "The Shortlist gathers submissions ready for closer decision-making. Notes and status context stay attached as the group narrows.", screenLabel: "Institution workspace · Shortlist", screenCue: "Look for shortlisted artists, finalist status, committee vote context, and export/reporting actions.", viewerAction: "This helps preserve why a submission moved forward, so review history is not lost after the meeting.", nextPreview: "Finish this walkthrough, or return to the open-call setup flow.", primaryActionLabel: "Finish walkthrough", requiredRole: "institution", targetAction: "move_to_shortlist" },
  ]),
]
...
export function getScenarioSteps(scenarioId: DemoGuideScenarioId): DemoGuideStep[] { return demoGuideSteps.filter((step) => step.scenarioId === scenarioId).sort((a, b) => a.stepNumber - b.stepNumber) }
export function getFirstStepForScenario(scenarioId: DemoGuideScenarioId): DemoGuideStep | undefined { const scenario = scenarioById.get(scenarioId); if (!scenario) return undefined; return stepById.get(scenario.firstStepId) }
export function getGuideStep(stepId: string | null | undefined): DemoGuideStep | undefined { if (!stepId) return undefined; return stepById.get(stepId) }
export function getNextGuideStep(currentStepId: string | null | undefined): DemoGuideStep | undefined { const current = getGuideStep(currentStepId); if (!current?.nextStepId) return undefined; return stepById.get(current.nextStepId) }
export function getPreviousGuideStep(currentStepId: string | null | undefined): DemoGuideStep | undefined { const current = getGuideStep(currentStepId); if (!current) return undefined; const steps = getScenarioSteps(current.scenarioId); const index = steps.findIndex((step) => step.id === current.id); if (index <= 0) return undefined; return steps[index - 1] }
export function getScenarioById(scenarioId: DemoGuideScenarioId | null | undefined): DemoGuideScenario | undefined { if (!scenarioId) return undefined; return scenarioById.get(scenarioId) }
export function isDemoGuideScenarioId(value: string | null | undefined): value is DemoGuideScenarioId { if (!value) return false; return scenarioById.has(value as DemoGuideScenarioId) }
export function getRecommendedScenariosForPath(pathname: string | null | undefined): DemoGuideScenario[] {
  ...
  if (path.startsWith("/dashboard") || path.startsWith("/signup/institution") || path.startsWith("/programs") || path.startsWith("/committee") || path.startsWith("/review-queue") || path.startsWith("/messages") || path.startsWith("/activity-log") || path.startsWith("/shortlist") || path.startsWith("/reports")) return [...institutionScenarios, ...artistScenarios]
  return demoGuideScenarios
}
export function getRecommendedNextScenarios(scenarioId: DemoGuideScenarioId | null | undefined): DemoGuideScenario[] { const scenario = getScenarioById(scenarioId); if (!scenario) return []; return scenario.recommendedNextScenarioIds.map((nextId) => getScenarioById(nextId)).filter((entry): entry is DemoGuideScenario => Boolean(entry)) }
