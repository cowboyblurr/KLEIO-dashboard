...
async function signedPreview(storagePath: string, mimeType: string) {
...
async function rowToItem(row: Record<string, unknown>): Promise<ArtistMediaLibraryItem> {
  ...
  return {
    ...
    previewUrl: await signedPreview(storagePath, mimeType),
...
export async function validateDeviceMedia(file: File, config: MediaImportConfig) {
  ...
  if (!config.allowedMimeTypes.includes(mimeType)) {
    throw new Error(`${file.name} is not accepted here. Choose ${readableAcceptedMedia(config.allowedMimeTypes)} supported by this step.`)
  }
