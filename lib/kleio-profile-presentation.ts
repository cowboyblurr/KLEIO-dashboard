...
async function signedArtistAssetUrl(path: string | null) {
  if (!path) return null
...
export async function loadArtistProfilePresentation(): Promise<ArtistProfilePresentationRecord> {
  ...
  const { data, error } = await supabase
    ...
    .select("profile_image_path, featured_work_id")
    .eq("user_id", account.user.id)
    .maybeSingle()

  if (error) throw error

  const profileImagePath = typeof data?.profile_image_path === "string" ? data.profile_image_path : null
  ...
  return {
    ...
    profile_image_url: await signedArtistAssetUrl(profileImagePath),
  }
}

export async function uploadArtistProfileImage(file: File) {
  if (!file.type.startsWith("image/")) throw new Error("Choose a JPG, PNG, or WebP profile image.")
  if (file.size > 12 * 1024 * 1024) throw new Error("Profile images must be 12 MB or smaller.")
  ...
  return {
    path,
    signedUrl: await signedArtistAssetUrl(path),
  }
}
